...
- `internal/wasmhost` — `js && wasm`-only host predicate: is this binary
  running under Node or in a browser page? Selects the durable SQLite VFS
  (`nodefs` vs `opfs`) for every wasm storage caller.
