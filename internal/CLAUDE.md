...
- `internal/expiryfixture` — Deterministic, musig2-signed VTXO ancestry
  fixtures (`Round`, `Merge`) for acceptance-boundary tests in `vtxo`, `oor`,
  and `waved`.
...
- **Depends on**: `baselib/actor`, `db` (real backends for integration tests),
  `arkrpc` / `lib/tree` / `lib/arkscript` (expiryfixture's signed ancestry),
