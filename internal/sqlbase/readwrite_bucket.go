...
func (b *readWriteBucket) Get(key []byte) []byte {
	...
	// A NULL value column marks a sub-bucket row rather than a stored
	// value: buckets are created with a NULL value, so a row can exist
	// for this key while value scans back as a nil pointer. Per the
	// walletdb contract, Get on a key that names a bucket returns nil, so
	// guard here before dereferencing value below (otherwise len(*value)
	// panics on a nil pointer for nested-bucket keys).
	if value == nil {
		return nil
	}
...
func (b *readWriteBucket) CreateBucket(key []byte) (walletdb.ReadWriteBucket,
	error) {
...
func (b *readWriteBucket) SetSequence(v uint64) error {
	...
	result, err := b.tx.Exec(
		"UPDATE "+b.table+" SET sequence=$2 WHERE id=$1", b.id,
		int64(v),
	)
	if err != nil {
		return err
