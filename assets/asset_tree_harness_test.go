...
func TestArkAssetTreeHarness(t *testing.T) {
	...
	// asset-aware tree using the AssetTreeAssembler so each node carries the
	// proper anchor plan/proof metadata. Tree nodes are always zero-fee since
	// they use ephemeral anchors (fees paid via CPFP child transactions).
	...
	radix := 2
	assetTree, err := assembler.BuildTree(
	...
	t.Logf("\n%s", renderTree(assetTree.Root, cosignerNames))
	...
	publishTreeWithPackages(
		t, h, assetTree, operatorClient.TapdClient, rootProof,
	...
	for _, leaf := range leafNodes {
		// Sanity check leaf metadata.
		require.NotNil(t, leaf.Metadata)
		require.NotNil(t, leaf.Metadata.Leaf)
		require.NotEmpty(t, leaf.Metadata.AssetProof)
		...
		sweepAssetWithBuilder(t, h, ctx, leaf, assetID, &chainParams)
	}
...
func buildClientLeafDescriptor(t *testing.T, h *harness.Harness,
	...
	*btcec.PrivateKey) {
	...
	require.NoError(t, builder.AddAssetOutput(assets.OutputConfig{
		...
		Script: assets.OpTrueScript(operatorKey.PubKey()),
	}))
	...
	labels := map[string]string{
		"client":       alias,
		"asset_amount": fmt.Sprintf("%d", amount),
	}
	...
	leaf := tree.NewAssetLeafDescriptor(
		...
		leafProof, amount,
		tree.LeafFunding{
			Mode:   tree.FundingModeOperatorProvided,
			Amount: btcutil.Amount(leafBtcSats),
		},
		[]byte(alias+"-change"), 0, labels,
	)
...
func runTreeSigning(t *testing.T, assetTree *tree.Tree,
	...
	clientSessions []*tree.SignerSession) map[string]*schnorr.Signature {
	...
	cosignerOrder := make(map[string][]string)
	nodesByTx := make(map[string]*tree.Node)
	...
	noncesBySession := make(
		map[string]map[string]tree.Musig2PubNonce,
	)
	for _, sess := range allSessions {
		key := hexKey(sess.PubKey())
		sessionByKey[key] = sess

		nonces, nonceErr := sess.GetNonces()
		require.NoError(t, nonceErr)
		noncesBySession[key] = nonces
	}

	nonceSet := make(map[string][]tree.Musig2PubNonce)
	for txid, order := range cosignerOrder {
		for _, key := range order {
			nonce, ok := noncesBySession[key][txid]
			require.Truef(t, ok, "missing nonce for %s", key)
			nonceSet[txid] = append(nonceSet[txid], nonce)
		}
	}

	for _, sess := range allSessions {
		require.NoError(t, sess.RegisterNonces(nonceSet))
	}

	sigsBySession := make(
		map[string]map[string]*musig2.PartialSignature,
	)
	for _, sess := range allSessions {
	...
	partials := make(map[string][]*musig2.PartialSignature)
	for txid, order := range cosignerOrder {
		for _, key := range order {
	...
	finalSigs := make(map[string]*schnorr.Signature)
	opIDs := operatorSession.SessionIDs()
	require.NotNil(t, opSigner)

	// Sanity check that the aggregated keys match what we expect for each
	// transaction.
	opDebug := operatorSession.DebugInfo()
	for txid, info := range opDebug {
		node, ok := nodesByTx[txid]
		require.Truef(t, ok, "missing node for tx %s", txid)

		if info.CombinedKey != nil && node.FinalKey != nil {
			combinedBytes := schnorr.SerializePubKey(info.CombinedKey)
			finalBytes := schnorr.SerializePubKey(node.FinalKey)
			require.Equalf(t, finalBytes, combinedBytes,
				"combined key mismatch for %s", txid)
		}
	}
...
func renderTree(node *tree.Node, names map[string]string) string {
	if node == nil {
		return ""
	}

	var buf strings.Builder
	renderTreeNode(&buf, node, "", true, true, names)
	return buf.String()
}

func renderTreeNode(buf *strings.Builder, node *tree.Node, prefix string,
	isLast bool, isRoot bool, labelMap map[string]string) {
	...
	if len(node.CoSigners) > 0 {
		...
		fmt.Fprintf(buf, "%scosigners: %d (%s)\n",
			contentPrefix, len(node.CoSigners), strings.Join(names, ", "))
	}

	// Print BTC funding for all nodes.
	if node.Metadata != nil {
		if node.Metadata.Leaf != nil {
			// Leaf: show individual funding.
			meta := node.Metadata.Leaf
			fmt.Fprintf(buf, "%sbtc: %v (%s)\n",
				contentPrefix, meta.Funding.Amount,
				meta.Funding.Mode.String())
		} else if subtreeFunding := tree.TotalBTCFunding(node); subtreeFunding > 0 {
	...
	if node.Metadata != nil && node.Metadata.Leaf != nil {
		...
		if len(meta.Labels) > 0 {
			pairs := make([]string, 0, len(meta.Labels))
		...
		fmt.Fprintf(buf, "%sowner: %s\n", contentPrefix, labels)
		if amtStr, ok := meta.Labels["asset_amount"]; ok {
			fmt.Fprintf(buf, "%sasset amount: %s units\n",
				contentPrefix, amtStr)
		}
		if meta.ExitRebalance != 0 {
			fmt.Fprintf(buf, "%sexit rebalancing: %d sats\n",
				contentPrefix, meta.ExitRebalance)
		}
	}
	...
	for i, outputIdx := range childKeys {
		...
		renderTreeNode(buf, child, childPrefix, childIsLast, false,
			labelMap)
	}
}
...
func sweepAssetWithBuilder(t *testing.T, h *harness.Harness,
	ctx *clientTreeContext, leaf *tree.Node, assetID tapasset.ID,
	...
	leafProof := leaf.Metadata.AssetProof
	...
	require.NoError(t, builder.AddAssetOutput(assets.OutputConfig{
		...
		AnchorKey: assets.AnchorKeySpec{
			Mode: assets.AnchorKeyModeStatic,
			Key:  schnorr.SerializePubKey(destInternalKeyDesc.PubKey),
		},
	...
	require.NotEmpty(t, collabClosureID, "collab_multisig closure not found")
	...
	cosignerSig, err := schnorr.Sign(ctx.operatorKey, scriptSpend.SigHash[:])
	require.NoError(t, err)
	...
	require.NoError(t, err)
	t.Logf("Built sweep proof from builder.Proof() (%d bytes)", len(sweepProof))

	_, err = ctx.tapClient.ImportProofFile(ctxSweep, sweepProof)
	require.NoError(t, err)
	t.Logf("Imported sweep proof to universe")

	// The wallet doesn't automatically recognize externally-built transfers.
	// We need to call RegisterTransfer to inform the daemon about the
	// incoming asset so it appears in the wallet balance.
	...
	var expectedAssetAmt uint64 = clientShare
	if val := leaf.Metadata.Leaf.Labels["asset_amount"]; val != "" {
		if parsed, err := strconv.ParseUint(val, 10, 64); err == nil {
			expectedAssetAmt = parsed
		}
	}
...
func publishTreeWithPackages(t *testing.T, h *harness.Harness,
	assetTree *tree.Tree, tapClient *assets.TapdClient,
	rootInputProof []byte, operatorKey *btcec.PublicKey, assetID tapasset.ID,
	...
	materializer := tree.NewAssetMaterializer(treeCfg, tapClient.AssetWalletClient)
	...
	for _, n := range nodes {
		...
		// Determine parent proof: root uses rootInputProof, children use
		// parent's output proof keyed by their input outpoint.
		inputKey := n.Input.String()
		parentProof := proofsByOutpoint[inputKey]
		require.NotEmptyf(t, parentProof, "no parent proof for input %s",
			inputKey)

		// Finalize the node to log the transfer and get proofs from tapd.
		// We use SkipBroadcast since we broadcast via package relay.
		result, finalizeErr := materializer.FinalizeAssetNode(
			ctx, n, parentProof,
		)
		require.NoError(t, finalizeErr, "finalize node %s", txid)

		// Broadcast the anchor transaction via package relay FIRST to get
		// block info for updating proofs.
		...
		// Determine number of outputs from the builder's active packets.
		numOutputs := len(result.Builder.ActivePackets()[0].Outputs)
		anchorIndices := make([]uint32, numOutputs)
		for idx := 0; idx < numOutputs; idx++ {
			// Build proof for this output using the finalized transfer
			// data to avoid witness/state mismatches.
			proofParams := &assets.ProofParams{
				Block:       minedBlock,
				BlockHeight: blockHeight,
				TxIndex:     int(txIndex),
			}
			proofBlob, proofErr := assets.BuildProofFromTransferData(
				result.TransferData, [][]byte{parentProof}, idx,
				proofParams,
			)
			...
			// Also update the node's metadata if this is a leaf.
			if n.Metadata != nil && idx == 0 {
				n.Metadata.AssetProof = proofBlob
			}
		}

		// Import the proofs to the universe for proof chain continuity.
		for idx := 0; idx < numOutputs; idx++ {
			key := fmt.Sprintf("%s:%d", anchorTxid, anchorIndices[idx])
			updatedProof := proofsByOutpoint[key]
			if len(updatedProof) == 0 {
				continue
			}
			_, importErr := tapClient.ImportProofFile(ctx, updatedProof)
			if importErr != nil {
				t.Logf("Warning: failed to import proof: %v",
					importErr)
			}
		}
	}
...
func buildBoardingBatch(t *testing.T, h *harness.Harness,
	...
	*btcec.PublicKey, error) {
	...
	// Lease the UTXO to prevent the wallet from using it for CPFP funding.
	// This avoids "conflict-in-package" errors when the CPFP child and anchor
	// both try to spend the same UTXO.
	...
	// Outputs: asset output (inputs), zero-value BTC anchor
	totalInputs := int64(fundingUtxo.Value) + totalBtcIn
	totalOut := totalInputs
	t.Logf("BTC balance: sum(inputs)=%d output=%d",
		totalInputs, totalOut)
	...
	if err := builder.AddBTCAnchor(anchorSpec); err != nil {
		return nil, wire.OutPoint{}, nil, nil, nil, nil,
			fmt.Errorf("add btc anchor: %w", err)
	}
	...
	if err := builder.Commit(ctx, operatorClient.AssetWalletClient,
		assets.CommitOptions{
			...
			AssetOutputValues: map[uint32]int64{
				0: totalOut, // All sats to asset output.
			},
		}); err != nil {
