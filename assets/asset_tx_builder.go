...
type InputConfig struct {
	...
	// Sequence optionally overrides the default sequence number used for
	// the anchor input. A zero value requests the default of
	// MaxTxInSequenceNum-2.
	Sequence uint32
}
...
func (b *AssetTxBuilder) Commit(ctx context.Context, wallet assetWalletClient,
	...
	for i := range anchorPsbt.UnsignedTx.TxIn {
		...
		if i < len(b.inputs) && b.inputs[i].cfg.Sequence != 0 {
			sequence = b.inputs[i].cfg.Sequence
		}

		anchorPsbt.UnsignedTx.TxIn[i].Sequence = sequence
	}
