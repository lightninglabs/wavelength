...
- **Depends on**: `lnd/lnwallet/chainfee` (fee rate types),
  `lnd/lntypes` (weight units), `btcutil` (Amount type for wallet balance).
- **Depended on by**: `db` (`LedgerStoreDB` implements `LedgerStore`),
  `rounds` (fee computation during registration), root `darepo` (wiring),
  `systest` (fee assertions), `client/round` (fee estimation).
...
- **Schedule is immutable once created.** Updates produce a new `Schedule`
  ...
  `Schedule` in place.
- **Ledger accounts match the seeded chart of accounts.** The five account
  constants (`treasury_wallet`, `deployed_capital`, `user_vtxo_claims`,
  `operator_revenue`, `mining_fees`) must match the seed data in
  `db/sqlc/migrations/000010_accounting.up.sql`.
- **Ledger event types match the DB catalog.** Use `capital_committed` and
  `round_sweep` (not `capital_deployed` / `capital_reclaimed`).
