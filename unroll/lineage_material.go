...
type LineageMaterial struct {
	...
	// ExtraNodes holds zero or more finalized non-tree transactions that bridge
	// or extend lineage beyond the tree fragments.
...
func (m *LineageMaterial) Validate() error {
	...
	for i, node := range m.ExtraNodes {
		...
		txid, err := node.TXID()
		if err != nil {
			return fmt.Errorf("%w: extra node %d: %v",
