...
// positiveOnlyReconciler models the current chainsource subscription surface:
// it can replay historical positives, but silence is not an authoritative
// unspent answer.
type positiveOnlyReconciler struct {
	*stubChainReconciler
	spendCalls int
}

// HasAuthoritativeNegativeEvidence reports that notifier silence cannot prove
// absence.
func (p *positiveOnlyReconciler) HasAuthoritativeNegativeEvidence() bool {
	return false
}

// SpentOutpoint records any attempted unanchored negative probe.
func (p *positiveOnlyReconciler) SpentOutpoint(ctx context.Context,
	outpoint wire.OutPoint) (fn.Option[SpendAnchor], error) {

	p.spendCalls++

	return p.stubChainReconciler.SpentOutpoint(ctx, outpoint)
}
...
// TestPositiveOnlyReconcilerSkipsUnanchoredSpendProbe verifies that an ordinary
// restart does not wait for notifier silence to prove the target unspent. With
// no persisted spend anchor there is no positive fact to reconcile; the actor
// re-arms its live spend watch and the durable relive guard protects the
// offline uncertainty window.
func TestPositiveOnlyReconcilerSkipsUnanchoredSpendProbe(t *testing.T) {
	proof := buildLinearProof(t)
	rootTxid := proof.RootTxids()[0]
	reconciler := &positiveOnlyReconciler{
		stubChainReconciler: &stubChainReconciler{
			confirmed: map[chainhash.Hash]ConfirmedAnchor{
				rootTxid: {
					Txid:   rootTxid,
					Height: 101,
	...
	err := reconcileCheckpoint(
		t.Context(), reconciler, proof, checkpoint,
	)
	require.NoError(t, err)
	require.Zero(t, reconciler.spendCalls)
	require.Equal(
		t, []chainhash.Hash{rootTxid}, checkpoint.State.ConfirmedTxids,
	)
}
