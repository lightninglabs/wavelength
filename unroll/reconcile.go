...
// NegativeEvidenceReconciler optionally declares whether fn.None is backed by
// an authoritative completed query. Reconcilers that do not implement this
// interface retain the full ChainReconciler contract and are assumed capable
// of proving both positive and negative answers.
type NegativeEvidenceReconciler interface {
	// HasAuthoritativeNegativeEvidence reports whether the implementation
	// can prove absence without inferring it from silence or a timeout.
	HasAuthoritativeNegativeEvidence() bool
}
...
func reconcileCheckpoint(ctx context.Context, reconciler ChainReconciler,
	...
	// Verify only a sweep checkpoint that already claims confirmation.
	// A merely broadcast sweep may legitimately be unconfirmed, so notifier
	// silence cannot distinguish its expected state from a reorg.
...
func reconcileExternalSpend(ctx context.Context, reconciler ChainReconciler,
	...
	// With no persisted positive there is nothing a positive-only notifier
	// can safely disprove. The live historical spend watch is re-armed
	// before Resume routes work; later canonicality Snapshot/Ready supplies
	// the authoritative negative barrier required by one-confirmation mode.
	if checkpoint.ProvisionalExternalSpend.IsNone() {
		capability, ok := reconciler.(NegativeEvidenceReconciler)
		if ok && !capability.HasAuthoritativeNegativeEvidence() {
			return nil
		}
	}
