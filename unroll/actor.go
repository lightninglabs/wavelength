...
type Config struct {
	...
	// ActorID is the durable actor mailbox ID. When empty it falls back to a
	// deterministic ID derived from the target.
	ActorID string
	...
	// Wallet provides sweep destination derivation and timeout-path signing.
	Wallet SweepWallet
...
func (b *behavior) Receive(ctx context.Context, msg Msg) fn.Result[Resp] {
	...
	default:
		return fn.Err[Resp](fmt.Errorf("unknown unroll message: %T", msg))
	}
}
...
func (b *behavior) stateResponse() *GetStateResp {
	...
	if job.PlannerState.Sweep.Txid != nil {
		txid := *job.PlannerState.Sweep.Txid
		resp.SweepTxid = &txid
	}
...
func (b *behavior) restoreCheckpoint(ctx context.Context) error {
	...
	if decoded.Version != checkpointVersion {
		return fmt.Errorf("unknown checkpoint version %d", decoded.Version)
	}
	...
	for i := range outbox {
		...
		case *EnsureReadyTransactions:
			for _, txid := range evt.Txids {
				...
				if err := b.ensureNodeConfirmed(ctx, txid, node); err != nil {
					return err
				}
			}
		...
		case *ReissueSweepConfirmation:
			...
			_, err := b.cfg.TxConfirmRef.Ask(
				ctx, &txconfirm.EnsureConfirmedReq{
					Tx: b.sweepTx,
					ConfirmationPkScript: append(
						[]byte(nil), b.sweepTx.TxOut[0].PkScript...,
					),
					Label: "unroll-sweep-" +
						b.cfg.TargetOutpoint.String(),
					Subscriber: b.notificationRef(),
...
func (b *behavior) currentState() (State, error) {
	if b.session != nil && b.session.FSM != nil {
		...
		if !ok {
			return nil, fmt.Errorf("unexpected unroll state %T", rawState)
		}
...
func (b *behavior) notifyRegistryIfTerminal(ctx context.Context) {
	...
	if job.PlannerState.Sweep.Txid != nil {
		msg.SweepTxid = copyHash(job.PlannerState.Sweep.Txid)
	}
...
func copyPlannerState(state unrollplan.State) unrollplan.State {
	copyState := unrollplan.State{
		ConfirmedTxids: append([]chainhash.Hash(nil), state.ConfirmedTxids...),
		InFlightTxids:  append([]chainhash.Hash(nil), state.InFlightTxids...),
		Sweep:          state.Sweep,
	}
...
// addHash appends a hash if it is not already present.
func addHash(hashes []chainhash.Hash, hash chainhash.Hash) []chainhash.Hash {
	if containsHash(hashes, hash) {
		return hashes
	}

	result := append(append([]chainhash.Hash(nil), hashes...), hash)
	sortHashes(result)
	return result
}
