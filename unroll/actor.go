...
// variants so the delivery store can codec them. This helper maps every
// reorg-aware lifecycle notification into the corresponding durable unroll
// message. Unknown variants still fail loudly.
...
// the full reorg-aware lifecycle. Unlike the foundation version of the
// unroll planner, this FSM has explicit reversible reorg and finality
// transitions, so neither notification may be dropped or collapsed into a
// repeated confirmation. Unknown future variants still terminate loudly.
...
// per actor lifetime.
//
// PhaseFailed always means FailReason has been set (proof-tx terminal failure
// or sweep retry budget exhausted), so the actor is terminal immediately.
//
...
// FinalityDepth = configured policy horizon + 1 at waved/server.go. So the
// child IS evicted once the sweep confirmation buries past the
// reorg-safety depth, not retained indefinitely.
//
// PhaseExternalSpendObserved is reversible by design and never reaches this
// function as terminal; SpendFinalizedMsg moves the actor to PhaseCompleted
// with ExternalSpendFinalized set so the consumed VTXO retires on chain.
//
...
func (b *behavior) notifyRegistryIfTerminal(ctx context.Context) {
	...
	trulyTerminal := phase == PhaseFailed ||
		(phase == PhaseCompleted &&
			(b.sweepFinalized || job.ExternalSpendFinalized))
	if !trulyTerminal {
		return
	}
	...
	// On a terminal failure — or once an external spend that finalized the
	// target out from under an in-flight exit is itself final — ask the
	// wallet backend to drop the job's broadcast-but-unconfirmed
	// transactions so a full-node wallet stops perpetually rebroadcasting a
	// proof (or sweep) tx that can never confirm now that the exit is over
	// (wavelength#609). Under reorg-safe exits an external target spend is
	// a reversible completion rather than a failure, so we defer the
	// cleanup to ExternalSpendFinalized: the still-in-flight proof tree is
	// just as abandoned as on the failure path, but only once the spend can
	// no longer reorg back out. A normal sweep completion is excluded — its
	// transactions confirmed on chain and are never rebroadcast. Gated by
	// its own once-flag so it is independent of the registry handoff below
	// and does not re-issue removals on every subsequent terminal tick.
	externalSpendFinal := phase == PhaseCompleted &&
		job.ExternalSpendFinalized
	if (phase == PhaseFailed || externalSpendFinal) &&
		!b.abandonedBroadcastsRemoved {
...
func (b *behavior) emitExitCostIfCompleted(ctx context.Context, phase Phase,
	...
	if phase != PhaseCompleted || job.ExternalSpendFinalized ||
		b.exitCostNotified ||
