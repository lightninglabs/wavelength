...
func processEventWithJob(ctx context.Context, job *JobState,
	...
	case *SweepBroadcastedEvent:
		nextJob.PlannerState.Sweep.Status = unrollplan.SweepStatusBroadcasted
...
func deriveStateTransition(_ context.Context, job *JobState,
	...
	if env == nil || env.Proof == nil || env.Planner == nil {
		return nil, fmt.Errorf("unroll environment must be fully populated")
	}
	...
	if reissue {
		...
		if job.PlannerState.Sweep.Status == unrollplan.SweepStatusBroadcasted {
	...
	switch {
	case snapshot.Done:
		return transitionWithOutbox(&Completed{Job: job.Copy()}, outbox), nil
	...
	case snapshot.CSV != nil && !snapshot.CSV.Ready:
		return transitionWithOutbox(&AwaitingCSV{Job: job.Copy()}, outbox),
			nil
