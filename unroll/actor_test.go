...
func (f *fakeTxConfirmRef) Ask(_ context.Context,
	...
	if state == txconfirm.TxStateConfirmed {
		...
		err := req.Subscriber.Tell(context.Background(), &txconfirm.TxConfirmed{
			Txid:        req.Tx.TxHash(),
			BlockHeight: height,
			NumConfs:    1,
		})
...
func (f *fakeTxConfirmRef) lastRequest(t *testing.T) *txconfirm.EnsureConfirmedReq {
	t.Helper()
...
func (f *fakeTxConfirmRef) emitConfirmed(t *testing.T, index int,
	...
	err := subscriber.Tell(context.Background(), &txconfirm.TxConfirmed{
		Txid:        txid,
		BlockHeight: height,
...
func (f *fakeTxConfirmRef) emitFailed(t *testing.T, index int,
	...
	err := subscriber.Tell(context.Background(), &txconfirm.TxFailed{
		Txid:   txid,
		Reason: reason,
	})
	require.NoError(t, err)
...
func (f *fakeChainSourceRef) Ask(_ context.Context,
	...
	case *chainsource.SubscribeBlocksRequest:
		req := msg.(*chainsource.SubscribeBlocksRequest)
		f.mu.Lock()
		f.blockRef = req.NotifyActor.UnwrapOr(nil)
		f.mu.Unlock()
		promise.Complete(fn.Ok[chainsource.ChainSourceResp](
	...
	case *chainsource.RegisterSpendRequest:
		req := msg.(*chainsource.RegisterSpendRequest)
		f.mu.Lock()
		f.spendRef = req.NotifyActor.UnwrapOr(nil)
		f.mu.Unlock()
		promise.Complete(fn.Ok[chainsource.ChainSourceResp](
...
// emitBlock delivers one block epoch to the subscribed actor.
func (f *fakeChainSourceRef) emitBlock(t *testing.T, height int32) {
	...
	require.NoError(t, ref.Tell(
		context.Background(),
		chainsource.BlockEpoch{Height: height},
	))
}
...
func newActorHarness(t *testing.T, proof *recovery.Proof,
	...
	*fakeTxConfirmRef, *memCheckpointStore) {
	...
	err := behavior.restoreCheckpoint(context.Background())
	require.NoError(t, err)
...
func mustDecodeCheckpoint(t *testing.T, store *memCheckpointStore,
	...
	checkpoint, err := store.LoadCheckpoint(context.Background(), actorID)
	require.NoError(t, err)
...
func TestStartUnrollSubmitsInitialFrontier(t *testing.T) {
	...
	require.Equal(t, proof.RootTxids()[0], txconfirmRef.lastRequest(t).Tx.TxHash())

	checkpoint, err := store.LoadCheckpoint(context.Background(), "unroll-test")
	require.NoError(t, err)
	require.NotNil(t, checkpoint)
}
...
func TestResumeReissuesInflightWork(t *testing.T) {
	...
	err = store.SaveCheckpoint(context.Background(), actor.CheckpointParams{
		ActorID:   "resume-test",
	...
	err = resumeBehavior.restoreCheckpoint(context.Background())
	require.NoError(t, err)
...
func TestStartUnrollMultiParentSubmitsAllRoots(t *testing.T) {
	...
	require.ElementsMatch(t, proof.RootTxids(), txconfirmRef.requestedTxids())
...
func TestResumeReissuesSweepConfirmation(t *testing.T) {
	...
	sweepTx, err := buildSweepTx(
		context.Background(), &fakeSweepWallet{}, &fakeChainSourceRef{},
		proof, desc, 0,
	)
	require.NoError(t, err)
	...
	raw, err := encodeCheckpoint(&actorCheckpoint{
		...
		State: unrollplan.State{
			...
			TargetConfirmHeight: copyHeight(108),
...
func TestSweepConfirmationCompletesActor(t *testing.T) {
	...
	require.NotNil(t, checkpoint.State.Sweep.ConfirmHeight)
}
...
func TestResumeCSVWaitDoesNotSweepUntilMature(t *testing.T) {
	...
	raw, err := encodeCheckpoint(&actorCheckpoint{
		...
		State: unrollplan.State{
			...
			TargetConfirmHeight: copyHeight(102),
		},
	})
	require.NoError(t, err)
...
func TestExternalSpendTerminatesActor(t *testing.T) {
	...
	chainSource := beh.cfg.ChainSource.(*fakeChainSourceRef)
...
func TestStartUnrollIsIdempotent(t *testing.T) {
	...
	time.Sleep(25 * time.Millisecond)
	require.Equal(t, 1, txconfirmRef.requestCountForTxid(proof.RootTxids()[0]))
}
