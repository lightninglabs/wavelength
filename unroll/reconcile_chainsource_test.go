...
// in waved/server.go bakes the per-actor target outpoint into the
...
// TestChainSourceReconcilerTimeoutFailsClosed proves notifier silence is
// reported as unavailable evidence for both confirmation and spend probes. It
// must never be converted into an objective absent or unspent answer.
func TestChainSourceReconcilerTimeoutFailsClosed(t *testing.T) {
	t.Parallel()
	...
	chainRef := chainsource.ChainSourceKey.Spawn(
		system, "chainsource-reconcile-timeout",
		chainsource.NewChainSourceActor(
			chainsource.ChainSourceConfig{
				Backend: backend,
				System:  system,
			},
	...
	reconciler := NewChainSourceReconciler(
		ChainSourceReconcilerConfig{
			ChainSource:    chainRef,
			Proof:          proof,
			CallerID:       "unroll-reconcile-timeout",
	...
	require.NotEmpty(t, rootTxids)
	confirmed, err := reconciler.ConfirmedTx(t.Context(), rootTxids[0])
	require.ErrorIs(t, err, ErrReconcileEvidenceUnavailable)
	require.True(t, confirmed.IsNone())

	spent, err := reconciler.SpentOutpoint(
		t.Context(), proof.TargetOutpoint(),
	)
	require.ErrorIs(t, err, ErrReconcileEvidenceUnavailable)
	require.True(t, spent.IsNone())
}
