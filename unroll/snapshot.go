...
const (
	...
	// checkpointExternalSpendFinalizedRecordType is optional; present
	// (value 1) when a target spend crossed policy finality. This prevents
	// restart from misclassifying an objectively consumed target as a
	// recoverable failure.
...
func encodeCheckpoint(value *actorCheckpoint) ([]byte, error) {
	...
	if value.ExternalSpendFinalized {
		finalized := uint8(1)
		records = append(
			records, tlv.MakePrimitiveRecord(
				checkpointExternalSpendFinalizedRecordType,
				&finalized,
			),
		)
	}
...
func decodeCheckpoint(raw []byte) (*actorCheckpoint, error) {
	...
	stream, err := tlv.NewStream(
		...
		tlv.MakePrimitiveRecord(
			checkpointReliveUnsafeRecordType, &reliveUnsafe,
		),
	)
	if err != nil {
		return nil, fmt.Errorf("create checkpoint stream: %w", err)
	...
	if _, ok := parsed[checkpointExternalSpendFinalizedRecordType]; ok {
		checkpoint.ExternalSpendFinalized = extFinalized != 0
	}
