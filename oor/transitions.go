...
func (s *Idle) ProcessEvent(ctx context.Context, event Event,
	...
	case *StartTransferEvent:
		...
		// If the FSM environment is already bound to a stable session id
		// (for example via `NewSessionFromSnapshot`), verify the derived
		// Ark txid matches.
		//
		// A mismatch here is a correctness bug. It implies either:
		// - non-deterministic tx building; or
		// - inconsistent reconstruction of inputs/recipients from state.
...
func (s *AwaitingArkSignatures) ProcessEvent(ctx context.Context, event Event,
	...
	case *ArkSignedEvent:
		if evt.ArkPSBT == nil || evt.ArkPSBT.UnsignedTx == nil {
			return nil, fmt.Errorf("signed ark psbt must be provided")
		}
...
func (s *RetryBackoff) ProcessEvent(ctx context.Context, event Event,
	...
	case *RetryDueEvent:
		...
		if s.ResumeSnapshot == nil {
			return nil, fmt.Errorf("internal: missing resume " +
				"snapshot")
		}

		resumeState, err := OutgoingStateFromSnapshot(
			s.ResumeSnapshot,
		)
		if err != nil {
			return nil, err
		...
		outbox, err := OutboxForState(resumeState)
		if err != nil {
			return nil, err
		}
