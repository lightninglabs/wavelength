...
- `ClientActorCfg` — Configuration for OORClientActor (OutboxHandler,
  ServerConn, PackageStore, DeliveryStore, VTXOManager, optional `LedgerSink fn.Option[ledger.Sink]` for fire-and-forget accounting emission, and optional `Log fn.Option[btclog.Logger]` propagated to the underlying `DurableActorConfig` for structured logging).
