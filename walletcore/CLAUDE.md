...
- `Wallet` — Core wallet struct embedding `input.Signer`. Provides key derivation, P2TR address generation, balance queries, and UTXO listing. Also implements `proofkeys.Backend` (via `ProofSigner` method). Chain-specific implementations embed this to satisfy `round.ClientWallet`. `Balance` reports the default account only (see invariants).
...
- `Wallet.Balance` is scoped to `lnwallet.DefaultAccountName`, not to every
  account. Imported taproot scripts (boarding and exit outputs brought in via
  `ImportTaprootScript`) live in btcwallet's *imported* account, are
  unspendable by the wallet's own key ring, and are already reported under the
  daemon's separate boarding balance fields. Passing `""` here would sum both
  accounts and overstate what the wallet can actually fund an exit or sweep
  with. Do not "simplify" the account argument back to the empty string.
