// verify-schema-registry parses the arkcli source using Go AST and
...
const cliPkg = "cmd/arkcli"

func main() {
	if len(os.Args) > 1 {
		fmt.Fprintln(os.Stderr,
			"usage: go run "+
				"scripts/verify-schema-registry/main.go")
		os.Exit(2)
	}
	...
	// cmd_*.go files. We build the path from the command tree
	// structure.
	...
	// Check MCP → schema: every MCP tool must have a schema
	// entry. The reverse is not required because some schema
	// entries may be CLI-only.
...
// mcpToSchema converts an MCP tool name (e.g. "trigger_batch") to
// the expected schema method name (e.g. "trigger-batch"). MCP tools
// use underscores while schema methods use hyphens.
...
// exists in setB. It does NOT check the reverse — setB may have
// extra entries.
...
func extractSchemaMethods(pkg *ast.Package) []string {
	...
	for _, file := range pkg.Files {
		ast.Inspect(file, func(n ast.Node) bool {
			...
			// composite literals with a "Method" key.
...
func extractMCPToolNames(pkg *ast.Package) []string {
	...
	for _, file := range pkg.Files {
		ast.Inspect(file, func(n ast.Node) bool {
			...
			// Match mcp.AddTool calls (generic form).
			sel, ok := call.Fun.(*ast.IndexExpr)
			if ok {
				s, ok := sel.X.(*ast.SelectorExpr)
				if ok {
...
// extractCobraLeafCommands walks all new*Cmd() functions and
// extracts the Use field from cobra.Command literals that also have
// a RunE field. Parent commands (no RunE) are tracked for path
// building by inspecting AddCommand calls.
func extractCobraLeafCommands(pkg *ast.Package) []string {
	// First pass: map function names to their cobra Use and
	// RunE status.
	type cmdInfo struct {
		use    string
		hasRun bool
	...
	for _, file := range pkg.Files {
		ast.Inspect(file, func(n ast.Node) bool {
			...
			ast.Inspect(fn.Body,
				func(inner ast.Node) bool {
					// Look for cobra.Command
					// composite literals.
					comp, ok :=
						inner.(*ast.CompositeLit)
					if ok {
						use, hasRun :=
							parseCobraLit(comp)
						if use != "" {
					...
					sel, ok :=
						call.Fun.(*ast.SelectorExpr) //nolint:ll
					if !ok ||
						sel.Sel.Name != "AddCommand" { //nolint:ll
					...
					for _, arg := range call.Args {
						c, ok :=
							arg.(*ast.CallExpr) //nolint:ll
						if !ok {
							continue
						}

						ident, ok :=
							c.Fun.(*ast.Ident) //nolint:ll
						if ok {
							children = append( //nolint:ll
								children,
								ident.Name,
							)
						}
	...
	// Build the command paths by walking the tree from root
	// commands.
	var leaves []string
...
// parseCobraLit extracts the Use field value and whether RunE is
// set from a cobra.Command composite literal.
func parseCobraLit(
	comp *ast.CompositeLit) (string, bool) {
