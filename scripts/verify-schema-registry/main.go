...
func main() {
	...
	// Check schema ↔ MCP (must be exact match).
	errors = append(errors,
		checkSync("schema registry", schemaMethods,
			"MCP tools", mcpTools,
			schemaTOMCP)...)
	...
	if len(errors) > 0 {
		fmt.Println("Schema registry drift detected:")
		fmt.Println()
		for _, e := range errors {
			fmt.Printf("  - %s\n", e)
		}
		fmt.Println()
		fmt.Printf("Found %d issue(s). Update schema_registry.go, "+
			"cmd_mcp.go, or cmd_*.go to fix.\n", len(errors))
		os.Exit(1)
	}

	fmt.Printf("OK: schema registry (%d methods), MCP tools "+
		"(%d), cobra commands (%d) are in sync.\n",
		len(schemaMethods), len(mcpTools),
		len(rpcCobraCommands))
}

// schemaTOMCP converts a schema method name (e.g. "wallet.create") to
// the expected MCP tool name (e.g. "wallet_create").
func schemaTOMCP(method string) string {
...
// checkSync verifies that every name in setA maps to an entry in setB
// and vice versa.
func checkSync(nameA string, setA []string,
	...
	aTransformed := make(map[string]string, len(setA))
	for _, a := range setA {
		t := transform(a)
		aTransformed[t] = a
	}
	...
	// Check B → A.
	for _, b := range setB {
		if _, ok := aTransformed[b]; !ok {
			errs = append(errs, fmt.Sprintf(
				"%s has %q but %s has no matching entry",
				nameB, b, nameA))
		}
	}
