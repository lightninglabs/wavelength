...
// SendRPC sends a request payload and returns its correlation id and
// idempotency key.
func (c *Client) SendRPC(ctx context.Context, service string, method string,
	req proto.Message, opts mailboxrpc.RPCOptions) (string, string, error) {
	...
	if !statusOK(resp.Status) {
		return "", "", statusError("Send", resp.Status)
	}

	return correlationID, idempotencyKey, nil
}
