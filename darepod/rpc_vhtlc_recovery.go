...
const (
	rpcDirectionPay         = daemonrpc.VHTLCRecoveryDirection_VHTLC_RECOVERY_DIRECTION_PAY
	rpcDirectionReceive     = daemonrpc.VHTLCRecoveryDirection_VHTLC_RECOVERY_DIRECTION_RECEIVE
	rpcDirectionServerIn    = daemonrpc.VHTLCRecoveryDirection_VHTLC_RECOVERY_DIRECTION_SERVER_IN
	rpcDirectionServerOut   = daemonrpc.VHTLCRecoveryDirection_VHTLC_RECOVERY_DIRECTION_SERVER_OUT
	rpcDirectionUnspecified = daemonrpc.VHTLCRecoveryDirection_VHTLC_RECOVERY_DIRECTION_UNSPECIFIED

	rpcActionClaim       = daemonrpc.VHTLCRecoveryAction_VHTLC_RECOVERY_ACTION_CLAIM
	rpcActionRefund      = daemonrpc.VHTLCRecoveryAction_VHTLC_RECOVERY_ACTION_REFUND_WITHOUT_RECEIVER
	rpcActionUnspecified = daemonrpc.VHTLCRecoveryAction_VHTLC_RECOVERY_ACTION_UNSPECIFIED

	rpcStateArmed          = daemonrpc.VHTLCRecoveryState_VHTLC_RECOVERY_STATE_ARMED
	rpcStateUnrollStarted  = daemonrpc.VHTLCRecoveryState_VHTLC_RECOVERY_STATE_UNROLL_STARTED
	rpcStateWaitingTarget  = daemonrpc.VHTLCRecoveryState_VHTLC_RECOVERY_STATE_WAITING_FOR_TARGET
	rpcStateWaitingCSV     = daemonrpc.VHTLCRecoveryState_VHTLC_RECOVERY_STATE_WAITING_FOR_CSV
	rpcStateBuildingExit   = daemonrpc.VHTLCRecoveryState_VHTLC_RECOVERY_STATE_BUILDING_EXIT_SPEND
	rpcStateExitBuilt      = daemonrpc.VHTLCRecoveryState_VHTLC_RECOVERY_STATE_EXIT_SPEND_BUILT
	rpcStateSubmittingExit = daemonrpc.VHTLCRecoveryState_VHTLC_RECOVERY_STATE_SUBMITTING_EXIT_SPEND
	rpcStatePendingConfirm = daemonrpc.VHTLCRecoveryState_VHTLC_RECOVERY_STATE_EXIT_SPEND_PENDING_CONFIRMATION
	rpcStateCompleted      = daemonrpc.VHTLCRecoveryState_VHTLC_RECOVERY_STATE_COMPLETED
	rpcStateCancelled      = daemonrpc.VHTLCRecoveryState_VHTLC_RECOVERY_STATE_CANCELLED
	rpcStateFailed         = daemonrpc.VHTLCRecoveryState_VHTLC_RECOVERY_STATE_FAILED
	rpcStateUnspecified    = daemonrpc.VHTLCRecoveryState_VHTLC_RECOVERY_STATE_UNSPECIFIED
)
...
func (r *RPCServer) GetVHTLCRecoveryStatus(ctx context.Context,
	...
	*daemonrpc.GetVHTLCRecoveryStatusResponse, error) {
	...
	if err != nil {
		if errors.Is(err, db.ErrVHTLCRecoveryJobNotFound) {
			return &daemonrpc.GetVHTLCRecoveryStatusResponse{}, nil
		}
...
func (r *RPCServer) ListVHTLCRecoveries(ctx context.Context,
	...
	*daemonrpc.ListVHTLCRecoveriesResponse, error) {
	...
	resp := &daemonrpc.ListVHTLCRecoveriesResponse{
		Statuses: make(
			[]*daemonrpc.VHTLCRecoveryStatus, 0, len(statuses),
		),
	}
...
func recoveryJobFromProto(req *daemonrpc.ArmVHTLCRecoveryRequest) (
...
// value.
func recoveryDirectionFromProto(direction daemonrpc.VHTLCRecoveryDirection) (
	string, error) {
...
// enum.
func recoveryDirectionToProto(
	direction string) daemonrpc.VHTLCRecoveryDirection {
...
func recoveryActionFromProto(action daemonrpc.VHTLCRecoveryAction) (string,
	error) {
...
func recoveryActionToProto(action string) daemonrpc.VHTLCRecoveryAction {
	switch action {
...
func recoveryStateToProto(state string) daemonrpc.VHTLCRecoveryState {
	switch state {
...
func recoveryStateIsTerminal(state daemonrpc.VHTLCRecoveryState) bool {
	switch state {
...
func recoveryStatusToProto(
	status *coordinator.RecoveryStatus) *daemonrpc.VHTLCRecoveryStatus {
