...
// DAREPOD_LWWALLET_SEED environment variable. It returns the seed and
// true if the variable is set and valid, or a zero seed and false
// otherwise.
func LoadSeedFromEnv() ([rawSeedLen]byte, bool) {
	hexSeed := os.Getenv(seedEnvVar)
	if hexSeed == "" {
		return [rawSeedLen]byte{}, false
	}

	decoded, err := hex.DecodeString(strings.TrimSpace(hexSeed))
	if err != nil || len(decoded) != rawSeedLen {
		return [rawSeedLen]byte{}, false
	}
