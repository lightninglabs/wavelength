...
// TestForfeitReleasedFromForfeitingUnboundRefused proves that a legacy or
// corrupt ForfeitingState with no recorded owner fails closed. Two empty IDs
// are not evidence that an unscoped producer owns the reservation.
func TestForfeitReleasedFromForfeitingUnboundRefused(t *testing.T) {
	t.Parallel()

	h := newVTXOTestHarness(t)
	vtxo := h.newTestDescriptor()
	h.withState(&ForfeitingState{VTXO: vtxo})
