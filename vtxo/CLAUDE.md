...
- **Spend reservations are epoch-stamped, and a stale release is refused.** `markReserved` stamps every in-memory spend reservation with a monotonic epoch, echoed to the owner on `SelectAndReserveSpendResponse.SelectedVTXOs[i].ReserveEpoch` and carried through `wallet.SelectedVTXO` into the spending session's durable state (see `oor.TransferInput.ReserveEpoch`). `handleReleaseSpend` consults `ReleaseSpendRequest.ReserveEpochs[outpoint]` and refuses the release — logging at `Info` and skipping the outpoint — only when a live in-memory reservation names a **different** non-zero epoch. That case means the reservation was superseded (released and re-reserved by a newer session in the same process), so honouring the stale release would hand a coin the newer session is actively spending back to the live set. Every other case releases: a zero caller epoch is "unknown" (a pre-upgrade snapshot, or a manual unlock) and releases unconditionally, and a missing live reservation means the manager restarted, where refusing would strand the coin in `Spending` forever.
- **The orphan sweep re-marks with epoch zero, deliberately.** `sweepOrphanedReservations` calls `markReservedUnknown` rather than `markReserved`: the epoch counter restarts at 0 on boot, so a fresh stamp would not match the epoch a resumed session persisted and that session's later release would be wrongly refused as superseded, stranding the coin. Admission only tests presence (`isReserved`), and the sweep's `Tell` carries no failure hop-back, so nothing on that path needs the real epoch. Do not "fix" this to allocate a fresh epoch.
