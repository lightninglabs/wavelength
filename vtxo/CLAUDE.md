...
- `ListLiveDescriptorsRequest` / `ListLiveDescriptorsResponse` — Ask-message pair for retrieving the descriptors of all VTXOs the manager recovered from durable state at `Start` time. The response slice is caller-owned. Used by daemon-local subsystems (notably the `fraud` watcher) to re-arm per-VTXO state on restart without taking a direct dependency on the VTXO store.
- `ManagerConfig.TerminalVTXOObserver` — Optional `func(context.Context, wire.OutPoint) error` callback invoked by the manager when a VTXO leaves its active set. Used by `darepod` to notify the `fraud.WatcherActor` to release watches for the terminal VTXO. Called synchronously in the manager's `handleVTXOTerminated` turn; failures are logged but do not block the terminal transition.
