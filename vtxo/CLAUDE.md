...
- `SumSpendableBalance(descs []*Descriptor)` — Returns the total confirmed balance across only the descriptors whose `Status` is `VTXOStatusLive`. Excludes spending, forfeiting, unilateral-exit, and other non-spendable statuses so callers (e.g. `darepod.GetBalance`) report only funds immediately available for sends.
- `VTXOActorConfig.FetchOperatorKey` — Optional `func(ctx) (*btcec.PublicKey, error)` seam on the VTXO actor config. When set, `refreshOutputTemplate` calls this at auto-refresh time to build a fresh policy template using the operator's current long-term key rather than reusing the stored key from the input VTXO. Nil preserves legacy/harness behavior (uses `EffectivePolicyTemplate`). A non-nil callback that errors propagates the failure so the next expiry tick retries.
