...
- `CheckForfeitAdmission(desc *Descriptor) error` — **advisory** pre-check
  (`forfeit_admission.go`) reporting whether a VTXO may be committed to a NEW
  cooperative forfeit (a leave, a refresh, or an in-round send). Nil means
  admissible. It exists so the predictable rejections carry an error naming
  the operation holding the coin instead of surfacing a raw FSM event-type
  mismatch to a user; it is **not** the authority (see the invariant below).
  Used by `waved.RPCServer.admitLeaveTargets`.
- Admission error sentinels (`admission_errors.go`), all matched with
  `errors.Is`: `ErrInsufficientSpendableFunds`, `ErrVTXOLiquidityLocked`
  (liquidity exists but is reserved by another in-flight operation),
  `ErrForfeitInFlight` (already committed to a round), `ErrExitInFlight`
  (handed to the chain resolver), `ErrVTXOTerminal` (no movable value left).
...
- **`CheckForfeitAdmission` is advisory, and the FSM stays the authority.** A reservation only settles once the VTXO actor's FSM has processed `PendingForfeitEvent`, and a concurrent claim (an automatic refresh cohort, say) can take the coin between the check and that event — so the FSM must keep refusing what it must, and this check must never be treated as having reserved anything. Two of its verdicts are deliberate and easy to get wrong when editing it:
  - **`VTXOStatusExpired` is admissible.** An expired VTXO's value is recoverable *only* by forfeiting it into an ordinary round, so refusing it here would strand the coin permanently. `ExpiredState` accepts `PendingForfeitEvent` for exactly that reason.
  - **`VTXOStatusPendingForfeit` is refused even though the FSM once tolerated the duplicate as a self-loop.** A second request carrying a different destination would otherwise report success while the first destination is what actually gets paid — and which of the two behaviours a caller met was decided purely by how far the round happened to have progressed in the preceding seconds.
  Rejections name the outpoint, the state, and (via `forfeitRoundHint`, once `ForfeitRoundID` is persisted) the round holding the coin, because they land in front of a user who asked to leave or refresh it.
