...
- **In-memory spend reservations are epoch-stamped (ABA guard).** `Manager.reserved` maps outpoint → a monotonic epoch stamped by `markReserved`; only the manager goroutine touches it. Two consumers depend on the epoch, both to distinguish "the reservation I observed" from "a later reservation on the same outpoint":
  - The asynchronous reservation-failure hop-back carries the epoch it saw and drops the mark via `dropReservedEpoch` **only when it still matches**, so a stale failure cannot un-gate a mark a newer reservation owns.
  - `handleReleaseSpend` refuses a release when `ReleaseSpendRequest.ReserveEpochs[op]` is non-zero *and* a live reservation names a different non-zero epoch — that reservation was superseded, so honouring the stale release would return to the live set a coin the newer session is actively spending. It logs at info (an expected redelivery outcome, not an operator alert) and skips that outpoint.

  Everything else releases: a zero epoch is "unknown" (a pre-upgrade snapshot or a manual unlock) and a missing live reservation means the manager restarted without repopulating the map (the orphan sweep skips `Spending` rows), so a resumed session's release must still be honoured rather than stranding the coin in `Spending` forever. `markReservedUnknown` exists for exactly that restart case: the epoch counter restarts at 0 so the sweep cannot reconstruct a resumed session's epoch, and a zero mark keeps the coin out of admission (`isReserved` tests presence) without letting the release guard refuse the resumed owner. Both helpers are nil-map-safe so test fixtures building a `Manager` literal without `NewManager` keep working.
