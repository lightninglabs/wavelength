...
- **Spend reservations carry a monotonic epoch, and a superseded release is
  refused.** `Manager.reserved` maps outpoint → epoch (stamped by
  `markReserved`), and the epoch rides out on
  `SelectAndReserveSpendResponse.SelectedVTXOs[].ReserveEpoch`. A caller that
  releases may name the epoch it held in
  `ReleaseSpendRequest.ReserveEpochs`; `handleReleaseSpend` refuses the release
  only when a live reservation names a *different* non-zero epoch, which means
  the coin was released and re-reserved by a newer owner (an ABA race — e.g. a
  rolled-back OOR failure redelivering its release after a newer session took
  the coin). Honouring that stale release would return to the live set a coin
  the newer session is actively spending. Refusal logs at info and skips the
  outpoint; it is not an error to the caller.
- **The epoch guard fails open in both of its unknown cases**, and must keep
  doing so. A zero epoch (a pre-upgrade snapshot with no epoch record, or a
  manual unlock that never held one) releases unconditionally, matching the
  pre-epoch behaviour. An outpoint with *no* live in-memory reservation also
  releases: after a restart the map is not repopulated for Spending rows (the
  orphan sweep skips them), so refusing there would strand the coin in
  `Spending` forever. `markReservedUnknown` exists for exactly this shape — the
  startup sweep marks a resumed session's coin reserved with epoch 0 so it
  stays out of admission (`isReserved` tests presence only) without letting the
  guard refuse that session's own release.
