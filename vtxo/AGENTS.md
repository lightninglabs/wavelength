...
- `ManagerConfig` — Configuration holding Store, Wallet, ChainSource, ActorSystem, ExpiryConfig, RoundActor ref, ChainResolver ref, optional `Log`, and optional `LedgerSink fn.Option[ledger.Sink]`. The manager propagates the sink into each spawned `VTXOActor` so per-VTXO handlers can fire-and-forget `ExitCostMsg` emissions. Also carries `ForfeitVTXOActorAskTimeout time.Duration` (zero → 5 s default; negative → no timeout) that bounds Ask calls on forfeit/refresh child actors so a blocked child cannot monopolize the shared manager admission point.
