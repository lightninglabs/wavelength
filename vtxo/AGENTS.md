...
- **Spend reservations are epoch-stamped to defeat ABA.** `Manager.markReserved` bumps a manager-goroutine-owned monotonic `reserveEpoch` and stamps it into the `reserved` map, returning the epoch on `SelectedVTXO.ReserveEpoch`. Two paths consume it. The asynchronous failure hop-back drops its in-memory mark only when the epoch still matches (`dropReservedEpoch`), so a stale failure from a released-then-re-reserved outpoint cannot un-gate a mark a newer reservation owns. `handleReleaseSpend` refuses a release only when a live in-memory reservation names a **different** non-zero epoch than the caller held: that means the reservation was superseded, so honouring the stale release would return a coin the newer owner is already spending to the live set. It logs at `Info` and continues rather than erroring. Everything else releases — a zero epoch is "unknown" (pre-upgrade snapshot or manual unlock), and *no* live reservation means the manager restarted with an unrepopulated map (the orphan sweep skips `Spending` rows), so a legitimately resumed session's release must still be honoured rather than stranding the coin. `markReservedUnknown` exists for exactly that startup case, since the epoch counter restarts at 0 and cannot reconstruct a resumed session's epoch.
