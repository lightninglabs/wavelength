...
- `ExitOutcome` — Terminal classification of an unroll job, reported back by
  `unroll`: `ExitOutcomeRecoverable` (failed with no on-chain footprint → roll
  back to `LiveState`), `ExitOutcomeConfirmed` (swept and confirmed → terminal
  `SpentState`), and `ExitOutcomeConflicted` (a confirmed foreign spend
  consumed a commitment output the exit depends on, so the exit is provably
  impossible — but the operator can only sweep that output past batch expiry,
  so the coin is expired, not lost, and routes to the non-terminal
  `ExpiredState` for reclaim by refresh, wavelength#1050).
...
- **A conflicted exit is reclaimed, not failed.** `ExitOutcomeConflicted`
  drives `ExitConflictedEvent` → `ExpiredState` (via `conflictExitedVTXO`),
  never a terminal `FailedState`. The distinction is load-bearing: the
  operator's sweep of the source batch output is only possible *past* batch
  expiry, so the coin is exactly an expired VTXO — quarantined from coin
  selection, excluded from `ListLiveVTXOs`, and reclaimable by forfeiting it
  into an ordinary round on the next block epoch. Retiring it to `FailedState`
  would strand recoverable value permanently. When no live actor exists (the
  usual restart case, since exiting VTXOs are outside the live-recovery set)
  the manager re-materializes one in `ExpiredState` from the persisted
  descriptor, and only if the row is *still* in the exit state — so a
  re-delivered conflict cannot stomp a VTXO that has since been reissued or
  recovered. A **recovery-only** target (`ExitPolicyKind.Valid()`) is the
  exception and is held in `UnilateralExitState`, mirroring
  `recoverExitedVTXO`'s guard: a swap-contract output is not spendable wallet
  liquidity, so refreshing it into a round would be wrong.
