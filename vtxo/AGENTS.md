...
- `ManagerConfig` — Configuration holding Store, Wallet, ChainSource,
  ...
  `ReservationStore`, and `ForfeitParticipantSigner` (propagated to each
  spawned `VTXOActor`). Confirmed exit-cost accounting is emitted by unroll
...
- `ForfeitParticipantSigner` — Function type `func(ctx, *ForfeitParticipantSignRequest) ([]*types.ForfeitParticipantSig, error)`. Optional hook on `VTXOEnvironment` (plumbed from `VTXOActorConfig`/`ManagerConfig`) that obtains keyed schnorr signatures from non-local participants for custom VTXO policies (e.g. vHTLC-style swaps). Called only after the round has assigned connector outputs, since the connector prevout is part of the taproot sighash. `ForfeitParticipantSignRequest` carries the VTXO, spend path, exact forfeit tx, and connector/forfeit output details the signer must bind to. Returned signatures are verified against the recomputed sighash before use; `participantForfeitSigs` also folds in the local wallet's own forfeit signature, rejecting duplicates and any external signature that reuses the local pubkey. The combined set is carried on `ForfeitSignatureSubmission.ParticipantVTXOSigs` and forwarded to `round` on `round.ForfeitSignatureResponse.ParticipantVTXOSigs`.
- `CustomForfeitInput` / `ActivateCustomForfeitInputsRequest` / `ActivateCustomForfeitInputsResponse` / `DropCustomForfeitInputsRequest` / `DropCustomForfeitInputsResponse` — Aliases for the canonical types in `actormsg`. A custom-policy VTXO (not yet a normal wallet-owned row, e.g. a counterparty's vHTLC leg) is activated as a temporary `PendingForfeitState` signer actor so it can co-sign a forfeit alongside the caller's own inputs, then dropped if the round intent never registers. If the outpoint has no durable row yet, activation persists a synthetic `VTXOStatusPendingForfeit` descriptor for the lifetime of the signer; if the row already exists (e.g. a recovered swap/refresh candidate), activation overlays the normal actor without mutating the row, so drop/rollback can restore the ordinary actor from storage untouched. `Manager.customForfeitSynthetic` (keyed by outpoint) tracks which overlays own a synthetic row so rollback knows whether to delete it or merely respawn the original actor.

## Relationships

- **Depends on**: `baselib/protofsm` (FSM engine), `baselib/actor` (actor system), `lib/tree` (tree paths), `lib/arkscript` (taproot construction and policy helpers in `IncomingVTXOHandler`), `lib/actormsg` (admission message types), `lib/types` (`ForfeitParticipantSig`), `arkrpc` (`IncomingVTXOEvent`), `chainsource` (block epochs), `ledger` (`Sink` type for compatibility with manager wiring), `unroll` (via `ExitOutcomeResolver` callback wired by `darepod`), `metrics` (optional `Sink` for `IncomingVTXOHandler` receive counters).
- **Depended on by**: `round` (triggers forfeit requests, drops custom forfeit input overlays on rollback), `oor` (incoming VTXOs), `wallet` (admission gating, activates/drops custom forfeit input overlays), `db` (persistence), `darepod` (wiring, owned-script adapters, incoming event route).
- **Sends**:
  - → `round` (via manager relay): `RelayToRoundMsg` wrapping `ForfeitSignatureSubmission` (carries `ParticipantVTXOSigs` for custom-policy forfeits)
  - → `db` (via outbox): `VTXOStatusUpdate`
  - → `vtxo` manager: `VTXOTerminatedNotification`, `RelayToRoundMsg`, `VTXOsMaterializedNotification` (from `IncomingVTXOHandler`), `spendReservationFailedMsg` (internal hop-back from a detached reserve watcher)
  - → `ledger` actor: no direct messages; unroll emits confirmed
    `ExitCostMsg` after sweep confirmation
  - → `metrics` actor (optional, from `IncomingVTXOHandler`): `OORTransferReceivedMsg`
- **Receives**:
  ...
  - ← `round` (via `lib/actormsg`): `DropCustomForfeitInputsRequest` (rollback of a registered-but-abandoned custom forfeit overlay)
...
- `selectAndReserveVTXOs` is a shared helper parameterized by `reserveParams` that serves both the OOR spend and cooperative forfeit coin selection paths, avoiding code duplication. It lists candidates via `VTXOStore.ListSelectionCandidatesByStatus`, a lightweight `(outpoint, amount, pkScript)` projection (`SelectedVTXO`), instead of decoding full descriptors on this per-payment hot path.
- **Detached (async) spend reservation.** The OOR spend path (`p.detached`) marks an in-memory reservation via `Manager.markReserved` (returns a monotonic per-outpoint epoch) and hands the reserve event to the child actor without blocking the manager turn on the child's FSM write transaction (`detachedReserve`). The outcome is observed on a detached goroutine via `Ask(...).OnComplete`; a failure (or an ambiguous timeout re-confirmed as still-`Live` in the store) hops back to the manager as a `spendReservationFailedMsg` carrying the observed epoch. `Manager.dropReservedEpoch` only clears the mark if the epoch still matches, so a stale failure cannot un-gate a reservation a newer session owns. The cooperative-forfeit path stays synchronous (`Ask` inline) since forfeit selection is not on the payment hot path.
...
- `IncomingVTXOHandler` optionally emits `metrics.OORTransferReceivedMsg` (via `IncomingVTXOHandlerConfig.MetricsSink`) once the terminal outcome of an owned incoming VTXO is known: `"materialized"` after the descriptor is persisted, `"failed"` when a relevant receive cannot be persisted. Pre-ownership ignore paths (non-`CREATED` events, malformed pushes, unowned scripts) emit nothing. Emission is best-effort/fire-and-forget and never fails the receive.
...
- `ForfeitReleasedEvent` is accepted in both `PendingForfeitState` and `ForfeitingState`. A round can fail during `ForfeitSignaturesCollecting` after a VTXO already replied with its forfeit signature and advanced to `ForfeitingState`; the release is still safe because no forfeit signature reaches the server until the success edge out of `ForfeitSignaturesCollecting` calls `SubmitVTXOForfeitSigsToServer`. Both cases return the VTXO to `LiveState` with `VTXOStatusUpdate{Live}` rather than leaving it wedged.
- Multi-participant forfeit signing: when a custom VTXO policy requires more than the local wallet signature (`evt.ForfeitSpend != nil` or external signatures are present), `participantForfeitSigs` requires both a local pubkey and signature, rejects duplicate or locally-colliding external pubkeys, and appends the local signature last. External signatures are schnorr-verified against the recomputed forfeit sighash (`forfeitParticipantSigHash`, built from the same `tx.VTXOSpendContext`/`tx.ConnectorSpendContext`/`tx.NewForfeitPrevOutFetcher` machinery as normal forfeit signing) before being trusted.
