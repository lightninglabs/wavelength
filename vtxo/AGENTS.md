...
- **Startup sweep of orphaned Spending VTXOs.** When `ManagerConfig.ReservationStore` is set, `Start` calls `sweepOrphanedReservations` after all actors are recovered. A Spending VTXO with no reservation row in the durable index is provably orphaned (its spend session died before checkpointing) and is released back to `LiveState` via `SpendReleasedEvent`. The sweep aborts entirely if `ListReservedOutpoints` fails to avoid releasing VTXOs an in-flight spend still owns. Coins the sweep re-marks as still-reserved go through `markReservedUnknown` (epoch 0), **not** `markReserved`: the epoch counter restarts at 0 on boot, so a fresh mark would not match the epoch the resumed session persisted and its later release would be wrongly refused as superseded, stranding the coin.
- **A spend release is scoped to the reservation it names.** `handleReleaseSpend` refuses an outpoint only when the caller passed a non-zero `ReleaseSpendRequest.ReserveEpochs` entry *and* a live in-memory reservation names a different non-zero epoch — the reservation was released and re-reserved by a newer owner in this process, so honouring the stale release would return to `LiveState` a coin the newer owner is spending. Every other shape releases: a zero epoch is "owner unknown" (pre-upgrade snapshot, manual unlock) and preserves the pre-epoch behaviour, and a missing live reservation means the manager restarted without repopulating the map for this coin (the orphan sweep skips rows that still have a reservation), so a resumed session's release must still be honoured rather than leaving the VTXO in `Spending` forever. The refusal is logged at `Info`: it is an expected redelivery outcome, not an operator-actionable fault.
- `selectAndReserveVTXOs` echoes each reserved outpoint's epoch back on `SelectedVTXO.ReserveEpoch`, and only the detached path (`p.detached`) stamps a real epoch — forfeit reservations carry zero. The epoch is per-process and monotonic, so it identifies *which* reservation a caller holds, not a durable identity that survives restart.
