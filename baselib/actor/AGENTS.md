...
- `MailboxWakeRegistrar` — Optional `DeliveryStore` interface (`RegisterMailboxWake(mailboxID, wake) (cancel func())`); a `DurableMailbox` registers on construction and cancels on `Close`. The store fires a mailbox's wake only after a transaction that enqueued into that mailbox commits, so the folded outbox-delivery path (where the target enqueue is invisible until the publisher's tx commits) restores same-process delivery latency. The wake is targeted by mailbox ID — only mailboxes actually enqueued into by the committed tx are roused, not every registered mailbox — which avoids an empty-peek storm under load. The poll ticker remains the cross-process/restart fallback.
