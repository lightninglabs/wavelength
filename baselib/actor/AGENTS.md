...
- `DurableActorConfig[M, R]` — Configuration struct for `DurableActor`: behavior (`fn.Either[ActorBehavior[M,R], BoundTxBehavior[M,R]]`; left = classic, right = Read/Commit path), store, codec, clock, DLO, WaitGroup, `TellRetryPolicy`, lease/heartbeat/poll durations, max attempts, cleanup timeout, and deduplication TTL. `NewDurableActor(cfg)` now returns `fn.Result[*DurableActor[M,R]]` — construction can fail if behavior is zero-valued, the store is not `TxAwareDeliveryStore` on the Read/Commit path, or a DurableAsk is attempted on the exec path.
...
- `DefaultDurableTxActorConfig[M, R, S]()` — Convenience constructor for actors on the Read/Commit path (see `TxBehavior`). Takes behavior, `StoreFactory[S]`, delivery store, and codec; returns a `DurableActorConfig` whose `Behavior` field is pre-populated with the bound `BoundTxBehavior`.
...
- `TxBehavior[M TLVMessage, R any, S any]` — Optional interface for behaviors that must perform side-effect IO (e.g. cross-actor Asks, network calls) **outside** the writer transaction. Instead of `Receive(ctx, msg M) fn.Result[R]`, the actor implements `Receive(ctx, msg M, ax Exec[S]) fn.Result[R]` and drives its own Read/Commit phases via the `Exec[S]` handle. The framework keeps the lease alive via heartbeats while `Receive` is executing. Use `DefaultDurableTxActorConfig` or `NewTxBehaviorEither` to register.
- `Exec[S any]` — Per-message execution handle passed to `TxBehavior.Receive`. Three methods: `Read(ctx, fn)` runs `fn` in a short read-only snapshot transaction; `Stage(ctx, fn)` runs `fn` in a lease-fenced writer tx without ack/dedup (for checkpoint-before-side-effect); `Commit(ctx, fn)` runs `fn` in a fenced writer tx with dedup + ack. Only one `Commit` per message; subsequent calls error.
- `StoreFactory[S any]` — Function type `func(context.Context, DeliveryStore) S` that builds the behavior's typed store `S` from a transaction-scoped `DeliveryStore`. Must NOT capture an outer context — the store must pick up the active `*sql.Tx` from the context passed into `Read`/`Stage`/`Commit` closures.
- `BoundTxBehavior[M, R]` — Type-erased container for a `TxBehavior[M, R, S]` + `StoreFactory[S]` pair. Fits in `DurableActorConfig.Behavior` without exposing the `S` type parameter. Constructed via `NewTxBehavior[M, R, S](behavior, factory)`.
...
- **Read/Commit (TxBehavior) invariants.** `Stage` and `Commit` are lease-fenced: if the lease is lost during IO, the operation returns `ErrLeaseLost` without applying state. `Stage` writes are durable but NOT ack'd — if the daemon crashes after `Stage` but before `Commit`, the message is redelivered and the behavior must be idempotent under replay against already-staged state. `Commit` is atomic (state + dedup + ack in one writer tx). `DurableAsk` is not supported on the exec path; callers attempting it receive `ErrDurableAskUnsupported`.
- **Lease heartbeat during exec.** While `TxBehavior.Receive` is executing, the framework periodically extends the lease so long-running side-effect IO does not cause the message to be rescheduled on another worker. The heartbeat loop terminates as soon as `Receive` returns.
