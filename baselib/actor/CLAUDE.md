...
- `DurableActorConfig[M, R]` — Configuration for durable actor creation. Includes `Log fn.Option[btclog.Logger]` for structured failure logging; wired at construction time by callers that hold a logger (e.g., `serverconn`, `oor`).
...
- `DurableActor` — Actor variant with crash-safe mailbox backed by SQL persistence. Provides `Wait(ctx)` to block until the actor stops and `StopAndWait(ctx)` to request a graceful shutdown and then wait. Logs transaction failures and Tell retries via the `Log fn.Option[btclog.Logger]` field on `DurableActorConfig` (falls back to `btclog.Disabled` when unset).
