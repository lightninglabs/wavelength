...
func NewTellOutboxEvent[M actor.Message, R any](key actor.ServiceKey[M, R], msg M) RoutedOutboxEvent[M, R] {
...
func NewAskOutboxEvent[M actor.Message, R any](key actor.ServiceKey[M, R], msg M) RoutedOutboxEvent[M, R] {
	return RoutedOutboxEvent[M, R]{
		key:  key,
		msg:  msg,
...
// TellRefEnv is an environment that can hold a tell-only actor reference.
// This let's the ActorStateMachine constructor set the reference after creation.
...
// FullRefEnv is an environment that can hold a full actor reference.
// This let's the ActorStateMachine constructor set the reference after creation.
type FullRefEnv[InternalEvent any, OutboxEvent ActorOutboxEvent, Env any] interface {
	SetActorRef(actor.ActorRef[ActorMessage[InternalEvent], ActorResponse[InternalEvent, OutboxEvent, Env]])
	GetActorRef() actor.ActorRef[ActorMessage[InternalEvent], ActorResponse[InternalEvent, OutboxEvent, Env]]
}

// SystemActorsStateMachine registers a new state machine actor and returns its reference.
func NewSystemsActorStateMachine[InternalEvent any, OutboxEvent ActorOutboxEvent, Env Environment](
	ctx context.Context, cfg StateMachineCfg[InternalEvent, OutboxEvent, Env], system *actor.ActorSystem,
	id string) actor.ActorRef[ActorMessage[InternalEvent], ActorResponse[InternalEvent, OutboxEvent, Env]] {
	...
	ref := actor.RegisterWithSystem(
		system, id, actor.NewServiceKey[ActorMessage[InternalEvent], ActorResponse[InternalEvent, OutboxEvent, Env]](id),
		sm,
	)
	extraInfo := ""
...
func (sm *ActorStateMachine[InternalEvent, OutboxEvent, Env]) Receive(ctx context.Context,
	e ActorMessage[InternalEvent]) fn.Result[ActorResponse[InternalEvent, OutboxEvent, Env]] {
	...
	if err != nil {
		return fn.NewResult(ActorResponse[InternalEvent, OutboxEvent, Env]{}, err)
	}
