...
`rpc/swapclientrpc`, `rpc/walletdkrpc`, and upstream
`btcwallet/rpc/walletrpc`, exposing every gRPC method as a subcommand with
auto-derived flags. Used by agents and power users to invoke any daemon RPC
...
- **Build-time generated registry.** The command tree is built at startup
  ...
  `walletdkrpc.File_wallet_proto`, and upstream btcwallet's
  `walletrpc.File_api_proto`. New gRPC methods require regenerating the
  registry (`go generate` or the corresponding make target) — they do NOT
  appear at runtime.
...
- **Depends on**: `daemonrpc` (service descriptors), `rpc/swapclientrpc`
  ...
  upstream `github.com/btcsuite/btcwallet/rpc/walletrpc` (service
  descriptors, for direct btcwallet introspection).
