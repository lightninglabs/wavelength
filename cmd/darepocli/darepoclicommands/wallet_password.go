...
// the same sources used for the wallet password: DAREPOD_SEED_PASSPHRASE
...
func readSeedPassphrase(cmd *cobra.Command) ([]byte, error) {
	if env := os.Getenv("DAREPOD_SEED_PASSPHRASE"); env != "" {
		return []byte(env), nil
	}
...
// priority order: DAREPOD_WALLET_PASSWORD env > --wallet_password_file
