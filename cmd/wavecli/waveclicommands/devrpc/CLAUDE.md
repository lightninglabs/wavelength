# cmd/wavecli/waveclicommands/devrpc

## Purpose

Dynamic low-level RPC CLI for daemon introspection. Constructs a `cobra`
command tree at init-time by walking proto descriptors from `waverpc` and
...
- **Build-time generated registry.** The command tree is built at startup
  ...
  produced at build time by `cmd/wavecli/internal/gen-devrpc` walking
  `waverpc.File_daemon_proto` and
...
- **Depends on**: `waverpc` (service descriptors), `rpc/swapclientrpc`
  (service descriptors).
- **Depended on by**: `cmd/wavecli/waveclicommands` (registered as the
  `dev` subcommand).
...
- The registry function in `registry_generated.go` is generated by
  `cmd/wavecli/internal/gen-devrpc` — **never edit it manually**.
