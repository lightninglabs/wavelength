...
func buildMCPServer(client waverpc.DaemonServiceClient,
	...
	// above the legacy waverpc tools so an agent listing tools
...
func registerMCPTools(s *mcp.Server, client waverpc.DaemonServiceClient) {
	...
	mcp.AddTool(s, &mcp.Tool{
		Name: "daemon.balance",
		Description: "Display the waverpc balance breakdown " +
			"(boarding + VTXO + total)",
	...
	// scripted access to the raw waverpc.NewAddress path.
	...
	mcp.AddTool(s, &mcp.Tool{
		...
		resp, err := client.NewReceiveScript(
			ctx, &waverpc.NewReceiveScriptRequest{
				Label: args.Label,
			},
		)
		if err != nil {
...
// the MCP server.
func registerMCPSendTools(s *mcp.Server, client waverpc.DaemonServiceClient) {
