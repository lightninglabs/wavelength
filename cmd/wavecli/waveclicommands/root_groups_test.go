...
// TestWavelengthDevRevealsAdvancedGroup verifies that dev mode surfaces the
// advanced subtrees under an Advanced group without disturbing the everyday
// face.
func TestWavelengthDevRevealsAdvancedGroup(t *testing.T) {
	t.Parallel()
...
// TestWavelengthDevEnvRevealsAdvanced verifies the WAVELENGTH_DEV env plumbing
// into NewRootCmd: only the exact value "1" reveals the advanced subtrees; any
// other value leaves them hidden. It intentionally does not run in parallel:
// t.Setenv is incompatible with t.Parallel.
func TestWavelengthDevEnvRevealsAdvanced(t *testing.T) {
	...
	for _, v := range []string{"0", ""} {
		...
		require.NotContainsf(
			...
			groupAdvanced, "WAVELENGTH_DEV=%q must not reveal "+
				"the advanced group",
			v,
		)
	}
