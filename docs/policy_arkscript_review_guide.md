...
recipient := RecipientOutput{
    Amount:         100_000,
    PolicyTemplate: vhtlcPolicy.Encode(),  // semantic, not raw pkScript
}
```
...
input := TransferInput{
    ...
    Amount:            100_000,
    VTXOPolicyTemplate: vhtlcPolicy.Encode(),
...
```go
// Server-side: oor/policy_helpers.go
...
```go
// Server-side: finalize_signature_validation.go
