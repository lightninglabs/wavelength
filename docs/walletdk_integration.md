...
The `swapdtest` harness can start a regtest topology and print a ready-made
`walletdk-tui` command:

```sh
make swapdtest-swapruntime
./swapdtest start
```
...
```sh
eval "$(./swapdtest aliases)"
walletdk-ui alice-ui
```

To inspect the exact SDK configuration flags without launching the TUI:

```sh
./swapdtest walletdk alice-ui --print
```

This environment is the fastest way to test the full flow:

1. Create a wallet in the TUI.
2. Create a receive invoice.
3. Pay it with `zane-payinvoice <invoice>`.
4. Watch swap accounting update.
5. Create a Zane invoice with `zane-lncli addinvoice --amt 50000`.
6. Pay that invoice from the wallet UI.
