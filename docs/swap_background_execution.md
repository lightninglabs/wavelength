...
Once #337 lands, the `swapclientserver` receive setup should configure the SDK
with the durable receive auth key and mailbox event receiver exposed there. The
current implementation probes for `SetReceiveAuthKey` so it can use the method
when available without requiring #337 on fresh `main`; the mailbox event
receiver wiring should be added when the SDK API is merged.
