...
| [credit_durable_actor_design.md](credit_durable_actor_design.md) | How the credit subsystem's client orchestration (top-up, credit receive, redeem) survives daemon restart and client disconnect: stable idempotency keys, durable daemon-owned execution, and reconciliation against the server credit ledger |
...
| [walletdk_mobile.md](walletdk_mobile.md) | `sdk/walletdk/mobile`, the gomobile-safe FFI facade over the embedded wallet SDK for Android/iOS hosts: build tags, JSON payload contract, and pull-based streaming |
