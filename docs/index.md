...
| [oor_subsystem.md](oor_subsystem.md) | The OOR (out-of-round) subsystem: per-session actor model, the `oor_session_registry` single source of truth, the Read/Commit turn with its inline outbox switch, outgoing/incoming flows, crash recovery, the per-session concurrency fix for issue #605, and the bounded keyset-paginated status reads backing `ListOORSessions`/`GetOORSession` |
