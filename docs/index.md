...
| [credit_durable_actor_design.md](credit_durable_actor_design.md) | How the credit subsystem's pay/receive/redeem flows survive daemon restart and client disconnect: crash-safe client orchestration atop the server-authoritative ledger |
...
| [walletdk_mobile.md](walletdk_mobile.md) | `sdk/walletdk/mobile`: the gomobile-safe JSON facade that lets Android/iOS hosts drive an embedded `darepod` wallet in-process |
