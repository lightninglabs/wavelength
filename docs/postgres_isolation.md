...
| `UpsertSession` ([`db/oor_session_registry_store.go`](../db/oor_session_registry_store.go)) | B (blind) | `UpsertOORSessionRegistry` targets `session_id`, which does not cover the partial unique `idx_oor_session_registry_idempotency_key`. Two fresh sessions racing on one key can both miss the immutable-attempt probe, so the session-row loser gets a `23505` and redelivers. `InsertOORDispatchAttempt` then uses targetless `ON CONFLICT DO NOTHING` across both permanent identity constraints and compares the winner inside the same transaction. |
