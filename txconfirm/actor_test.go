...
type fakeChainSourceRef struct {
	...
	confNotify  map[chainhash.Hash]actor.TellOnlyRef[chainsource.ConfirmationEvent]
...
func newFakeChainSourceRef(bestHeight int32) *fakeChainSourceRef {
	return &fakeChainSourceRef{
		...
		feeRate:          5,
		confNotify:       make(map[chainhash.Hash]actor.TellOnlyRef[chainsource.ConfirmationEvent]),
		confConfs:        make(map[chainhash.Hash]uint32),
		alreadyConfirmed: make(map[chainhash.Hash]chainsource.ConfirmationEvent),
	}
}
...
func (f *fakeChainSourceRef) handleAsk(_ context.Context,
	...
	default:
		return nil, fmt.Errorf("unsupported chainsource message %T", msg)
	}
}
...
func (f *fakeChainSourceRef) emitConfirmation(t *testing.T,
	...
	err := notifyRef.Tell(context.Background(), chainsource.ConfirmationEvent{
		Txid:        txid,
...
func (f *fakeChainSourceRef) emitBlock(t *testing.T, height int32) {
	...
	require.NotNil(t, notifyRef)
	err := notifyRef.Tell(context.Background(), chainsource.BlockEpoch{
		Height: height,
	})
	require.NoError(t, err)
}
...
func TestCancelInterestStopsTracking(t *testing.T) {
	...
	ref, _ := newTestActor(t, Config{
		ChainSource:           chain,
		Wallet:                &fakeWallet{utxos: []*wallet.Utxo{makeWalletUTXO()}},
