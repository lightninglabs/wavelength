...
type Wallet interface {
	...
	// NewWalletPkScript returns a fresh wallet-managed pkScript suitable for
	// change outputs.
...
func (b *CPFPBroadcaster) broadcastWithCPFP(ctx context.Context,
	...
	if err != nil {
		return nil, fmt.Errorf("select fee input: %w", err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("build cpfp child: %w", err)
	}

	if err := b.signCPFPChild(ctx, child, anchorOutput, feeInput); err != nil {
		return nil, fmt.Errorf("sign cpfp child: %w", err)
	}
	...
	if pkgErr != nil {
		if IsIgnorableBroadcastError(pkgErr) {
			b.log.DebugS(ctx, "Package already known for "+txid.String())
		} else if isPackageSubmissionUnsupported(pkgErr) {
			if err := b.broadcastIndividually(
				ctx, req.Tx, child, req.Label,
			); err != nil {
				return nil, fmt.Errorf("broadcast fallback: %w", err)
			}
		} else {
...
func (b *CPFPBroadcaster) selectFeeInput(ctx context.Context,
	...
	for _, utxo := range utxos {
		if _, excluded := b.usedFeeOutpoints[utxo.Outpoint]; excluded {
			continue
		}
...
func (b *CPFPBroadcaster) broadcastIndividually(ctx context.Context,
	...
	if parentErr != nil && !IsIgnorableBroadcastError(parentErr) {
		return fmt.Errorf("broadcast parent %s: %w", parentTxid, parentErr)
	}
...
func SelectFeeInput(inputs []FeeInput, minValue btcutil.Amount,
	...
	if best == nil {
		return nil, fmt.Errorf("no confirmed fee input with at least %d "+
			"sat", int64(minValue))
	}
