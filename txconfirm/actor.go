...
const (
	// DefaultFeeBumpIntervalBlocks is the default number of new blocks to wait
	// before retrying a still-unconfirmed transaction with a fresh CPFP child.
	DefaultFeeBumpIntervalBlocks int32 = 2
)

// Config configures the generic shared tx confirmation actor.
type Config struct {
	// ChainSource provides the blockchain interface for best-height queries,
...
func (a *TxBroadcasterActor) handleEnsure(ctx context.Context,
	...
	if existing, ok := a.tracked[txid]; ok {
		return a.attachExistingSubscriber(ctx, existing, req.Subscriber), nil
	}
	...
	if err := a.ensureBlockSubscription(ctx); err != nil {
		a.failTrackedTx(ctx, entry, fmt.Sprintf("subscribe blocks: %v", err))
		return a.ensureResp(entry, true), nil
	}

	if err := a.registerConfWatch(ctx, entry); err != nil {
		a.failTrackedTx(ctx, entry, fmt.Sprintf("register conf: %v", err))
		return a.ensureResp(entry, true), nil
	}

	if err := a.broadcastTrackedTx(ctx, entry, TxStateBroadcasting); err != nil {
...
func (a *TxBroadcasterActor) handleBlockObserved(ctx context.Context,
	...
	for _, entry := range a.tracked {
		...
		if err := a.broadcastTrackedTx(ctx, entry, TxStateFeeBumping); err != nil {
...
func (a *TxBroadcasterActor) attachExistingSubscriber(ctx context.Context,
	entry *trackedTx, subscriber actor.TellOnlyRef[Notification]) *EnsureConfirmedResp {
...
// use.
func (a *TxBroadcasterActor) ensureBlockSubscription(ctx context.Context) error {
...
func (a *TxBroadcasterActor) unregisterConfWatch(ctx context.Context,
	...
	if err != nil {
		return fmt.Errorf("unregister conf %s: %w", entry.data.Txid, err)
	}
