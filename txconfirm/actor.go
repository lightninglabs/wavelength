...
func (a *TxBroadcasterActor) handleConfirmationObserved(ctx context.Context,
	...
	if isTerminalTxState(state) {
		if a.retryLifecycleNotifications(ctx, entry) {
			a.evictTerminal(ctx, entry)
		}
...
func (a *TxBroadcasterActor) handleBlockObserved(ctx context.Context,
	...
	for _, entry := range a.tracked {
		...
		if state == TxStateConfirmed {
			a.retryLifecycleNotifications(ctx, entry)
...
// retryLifecycleNotifications retries pending reliable lifecycle
// notifications for a tracked transaction. Confirmed remains live after a
// retry; Finalized and Failed are evicted once every delivery succeeds.
func (a *TxBroadcasterActor) retryLifecycleNotifications(ctx context.Context,
...
// actor tick from retryLifecycleNotifications until every
