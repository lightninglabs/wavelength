...
func TestTrackedTxFSMInitialBroadcastFlow(t *testing.T) {
	...
	_, err := fsm.AskEvent(
		context.Background(), &trackedTxBroadcastStarted{},
	...
	_, err = fsm.AskEvent(
		context.Background(), &trackedTxBroadcastAccepted{
			Progress: progress,
		},
	).Await(context.Background()).Unpack()
	require.NoError(t, err)
	...
	_, err = fsm.AskEvent(
		context.Background(), &trackedTxConfirmed{
			BlockHeight: 102,
		},
	).Await(context.Background()).Unpack()
	require.NoError(t, err)
...
func TestTrackedTxFSMFeeBumpFlow(t *testing.T) {
	...
	_, err = fsm.AskEvent(
		context.Background(), &trackedTxFeeBumpStarted{},
	).Await(context.Background()).Unpack()
	require.NoError(t, err)
...
func TestTrackedTxFSMFailureAndInvalidTransitions(t *testing.T) {
	...
	_, err := fsm.AskEvent(
		context.Background(), &trackedTxFailed{
