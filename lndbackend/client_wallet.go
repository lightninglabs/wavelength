...
func (c *ClientWallet) signOutputRawWithLocator(ctx context.Context,
	...
	for i, signDesc := range signDescriptors {
		...
		if signDesc.KeyDesc.PubKey != nil {
			keyDesc.RawKeyBytes = signDesc.KeyDesc.PubKey.SerializeCompressed()
		}
		...
		if signDesc.KeyDesc.KeyLocator.Family != 0 ||
			...
			keyDesc.KeyLoc = &signrpc.KeyLocator{
				KeyFamily: int32(signDesc.KeyDesc.KeyLocator.Family),
				KeyIndex:  int32(signDesc.KeyDesc.KeyLocator.Index),
			}
		}
		...
		rpcSignDescs[i] = &signrpc.SignDescriptor{
			...
			SignMethod:    lndclient.MarshalSignMethod(signDesc.SignMethod),
