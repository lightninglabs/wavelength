...
func TestConfigEndpointDefaults(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			wantArkHost:  defaultMainnetServerGRPCHost,
			wantSwapHost: defaultMainnetSwapServerGRPCHost,
		},
		{
			name:         "mainnet rest",
			network:      "mainnet",
			transport:    RPCTransportREST,
			wantArkHost:  defaultMainnetServerRESTHost,
			wantSwapHost: defaultMainnetSwapServerRESTHost,
		},
		{
			name:         "testnet3 grpc",
			network:      "testnet",
