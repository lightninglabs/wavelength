...
func (c *Config) Validate() error {
	...
	// A zero depth resolves to the built-in default, which would silently
	// diverge from the server's horizon (where zero means immediate
	// finality). When the fail-closed batch-canonicality gate is active,
	// require an explicit non-zero depth so the same horizon is configured
	// deliberately on both sides rather than defaulted on one and
	// disabled/defaulted differently on the other.
	if c.ReorgSafetyDepth == 0 && c.BatchCanonicalityGate {
		return fmt.Errorf("reorgsafetydepth must be set explicitly " +
			"(> 0) when batchcanonicalitygate is enabled, so the " +
			"reorg-safety horizon matches the server")
	}
...
// /home/user/.waved/data/regtest). Path fields are normalized by
...
// directory. For example, "~/.waved" becomes "/home/user/.waved".
...
func expandTilde(path string) (string, error) {
	...
	// this, "~/.waved" would produce path[1:] == "/.waved"
