...
- `ServerInfo` is a bootstrap-time operator-terms snapshot, including the
  ...
  through yet. `MinConfirmations` is the boarding-input minimum, while
  `VTXOConfirmations` is the separate depth at which round-created VTXOs
  become spendable off-chain — do not read one for the other.
- `ListVTXOs(ctx, nil)` is **not** "no filter": it lists the inventory set —
  every VTXO except forfeited and spent ones, newest first — of which only
  live entries are spendable. Callers that need a specific subset must pass
  `ListVTXOsRequest.Statuses` explicitly.
