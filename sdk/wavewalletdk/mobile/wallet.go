...
const (
	// defaultReadTimeout bounds read-only wallet calls whose foreign host cannot
	// supply context.Context through gomobile. Reads are safe to repeat after a
	// timeout and must not inherit the daemon's entire lifetime.
	defaultReadTimeout = 10 * time.Second

	// defaultReceiveTimeout bounds invoice creation when a mobile host omits
	// TimeoutSeconds. A timed-out request has an uncertain outcome: callers
	// must reconcile Activity before deliberately creating another invoice.
...
func readContext(parent context.Context) (context.Context,
	context.CancelFunc) {
...
func decodeReceiveRequest(reqJSON []byte) (wavewalletdk.ReceiveRequest,
	...
	if mobileReq.TimeoutSeconds < 0 {
		return wavewalletdk.ReceiveRequest{}, 0,
			fmt.Errorf("receive timeout seconds must not be negative")
	}
	maxTimeoutSeconds := int64(maxReceiveTimeout / time.Second)
	if mobileReq.TimeoutSeconds > maxTimeoutSeconds {
		return wavewalletdk.ReceiveRequest{}, 0, fmt.Errorf(
			"receive timeout seconds %d exceeds maximum %d",
