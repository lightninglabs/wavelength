...
// enum.
func listViewToProto(v ListView) (wavewalletrpc.ListView, error) {
	switch v {
	case ListViewActivity, "":
		return wavewalletrpc.ListView_LIST_VIEW_ACTIVITY, nil

	case ListViewVTXOs:
		return wavewalletrpc.ListView_LIST_VIEW_VTXOS, nil

	case ListViewOnchain:
		return wavewalletrpc.ListView_LIST_VIEW_ONCHAIN, nil

	default:
		return wavewalletrpc.ListView_LIST_VIEW_UNSPECIFIED,
...
func listResultFromProto(view ListView,
	resp *wavewalletrpc.ListResponse) *ListResult {
...
func prepareSendResultFromProto(
	resp *wavewalletrpc.PrepareSendResponse) *PrepareSendResult {
...
func sendRailFromProto(rail wavewalletrpc.SendRail) SendRail {
	switch rail {
	...
	case wavewalletrpc.SendRail_SEND_RAIL_CREDIT:
		return SendRailCredit

	case wavewalletrpc.SendRail_SEND_RAIL_MIXED:
...
func creditPreviewFromProto(
	preview *wavewalletrpc.CreditPreview) *CreditPreview {
...
func quoteStatusFromProto(
	status wavewalletrpc.SendQuoteStatus) SendQuoteStatus {

	switch status {
	case wavewalletrpc.SendQuoteStatus_SEND_QUOTE_STATUS_COMPLETE:
		return SendQuoteStatusComplete

	case wavewalletrpc.SendQuoteStatus_SEND_QUOTE_STATUS_LOCAL_ONLY:
...
// exitJobStatusFromProto maps the wavewalletrpc ExitJobStatus enum onto the
// SDK string set.
func exitJobStatusFromProto(s wavewalletrpc.ExitJobStatus) ExitJobStatus {
	switch s {
	case wavewalletrpc.ExitJobStatus_EXIT_JOB_STATUS_PENDING:
		return ExitJobStatusPending

	case wavewalletrpc.ExitJobStatus_EXIT_JOB_STATUS_MATERIALIZING:
		return ExitJobStatusMaterializing

	case wavewalletrpc.ExitJobStatus_EXIT_JOB_STATUS_CSV_PENDING:
		return ExitJobStatusCSVPending

	case wavewalletrpc.ExitJobStatus_EXIT_JOB_STATUS_SWEEPING:
		return ExitJobStatusSweeping

	case wavewalletrpc.ExitJobStatus_EXIT_JOB_STATUS_COMPLETED:
		return ExitJobStatusCompleted

	case wavewalletrpc.ExitJobStatus_EXIT_JOB_STATUS_FAILED:
...
// exitStatusResultFromProto projects the wavewalletrpc ExitStatus response onto
...
func exitStatusResultFromProto(
	resp *wavewalletrpc.ExitStatusResponse) *ExitStatusResult {
...
func exitProgressFromProto(p *wavewalletrpc.ExitProgress) *ExitProgress {
	if p == nil {
		return nil
	}
...
// target confirms.
func exitCSVFromProto(c *wavewalletrpc.ExitCSV) *ExitCSV {
	if c == nil {
		return nil
	}
...
func exitFeesFromProto(f *wavewalletrpc.ExitFees) *ExitFees {
	if f == nil {
		return nil
	}
...
// exitSummaryFromProto projects the wavewalletrpc ExitSummary response onto its
// SDK DTO.
func exitSummaryFromProto(
	resp *wavewalletrpc.ExitSummaryResponse) *ExitSummaryResult {
...
func exitPlanFromProto(
	r *wavewalletrpc.GetExitPlanResponse) *GetExitPlanResult {
...
func exitPlanEntryFromProto(e *wavewalletrpc.ExitPlanEntry) ExitPlanEntry {
...
func exitInfeasibilityReasonFromProto(
	r wavewalletrpc.ExitInfeasibilityReason) ExitInfeasibilityReason {

	switch r {
	case wavewalletrpc.ExitInfeasibilityReason_EXIT_INFEASIBILITY_REASON_SWEEP_BELOW_DUST: //nolint:ll
		return ExitInfeasibilityReasonSweepBelowDust

	case wavewalletrpc.ExitInfeasibilityReason_EXIT_INFEASIBILITY_REASON_UNECONOMICAL: //nolint:ll
		return ExitInfeasibilityReasonUneconomical

	case wavewalletrpc.ExitInfeasibilityReason_EXIT_INFEASIBILITY_REASON_WALLET_UNDERFUNDED: //nolint:ll
...
func sweepWalletFromProto(
	resp *wavewalletrpc.SweepWalletResponse) *SweepWalletResult {
...
func entryFromProto(entry *wavewalletrpc.WalletEntry) Entry {
	if entry == nil {
		return Entry{}
	}
...
func entryProgressFromProto(
	progress *wavewalletrpc.WalletEntryProgress) *EntryProgress {
...
// entryPhaseFromProto maps the generated lifecycle enum into wavewalletdk's
// string-like phase, decoupled from proto enum renumbering.
func entryPhaseFromProto(phase wavewalletrpc.WalletEntryPhase) EntryPhase {
	switch phase {
	case wavewalletrpc.WalletEntryPhase_WALLET_ENTRY_PHASE_REQUEST_CREATED:
		return EntryPhaseRequestCreated

	case wavewalletrpc.WalletEntryPhase_WALLET_ENTRY_PHASE_WAITING_FOR_PAYMENT: //nolint:ll
		return EntryPhaseWaitingForPayment

	case wavewalletrpc.WalletEntryPhase_WALLET_ENTRY_PHASE_PAYMENT_DETECTED:
		return EntryPhasePaymentDetected

	case wavewalletrpc.WalletEntryPhase_WALLET_ENTRY_PHASE_SETTLING:
		return EntryPhaseSettling

	case wavewalletrpc.WalletEntryPhase_WALLET_ENTRY_PHASE_CONFIRMED:
		return EntryPhaseConfirmed

	case wavewalletrpc.WalletEntryPhase_WALLET_ENTRY_PHASE_REFUNDING:
		return EntryPhaseRefunding

	case wavewalletrpc.WalletEntryPhase_WALLET_ENTRY_PHASE_REFUNDED:
		return EntryPhaseRefunded

	case wavewalletrpc.WalletEntryPhase_WALLET_ENTRY_PHASE_FAILED:
		return EntryPhaseFailed

	case wavewalletrpc.WalletEntryPhase_WALLET_ENTRY_PHASE_WAITING_FOR_CONFIRMATION: //nolint:ll
...
func entryRequestFromProto(
	request *wavewalletrpc.WalletEntryRequest) *EntryRequest {
	...
	case *wavewalletrpc.WalletEntryRequest_LightningInvoice:
		return &EntryRequest{
	...
	case *wavewalletrpc.WalletEntryRequest_ArkAddress:
		return &EntryRequest{
...
// wavewalletdk's string-like code, decoupled from proto enum renumbering.
func entryFailureCodeFromProto(
	code wavewalletrpc.EntryFailureCode) EntryFailureCode {
	...
	case wavewalletrpc.
		EntryFailureCode_ENTRY_FAILURE_CODE_NEEDS_INTERVENTION:
...
// entryKindFromProto maps the generated enum into wavewalletdk's string-like
// kind.
func entryKindFromProto(kind wavewalletrpc.EntryKind) EntryKind {
	switch kind {
...
func entryKindToProto(kind EntryKind) (wavewalletrpc.EntryKind, error) {
	switch kind {
	case EntryKindSend:
...
func entryStatusFromProto(status wavewalletrpc.EntryStatus) EntryStatus {
	switch status {
...
func entryKindsToProto(kinds []EntryKind) ([]wavewalletrpc.EntryKind, error) {
	if len(kinds) == 0 {
		return nil, nil
	}

	out := make([]wavewalletrpc.EntryKind, 0, len(kinds))
...
func balanceFromProto(balance *wavewalletrpc.BalanceResponse) Balance {
	if balance == nil {
		return Balance{}
	}
