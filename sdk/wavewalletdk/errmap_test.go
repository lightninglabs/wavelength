...
// TestReconstructSentinel turns each wavewalletdk ErrorInfo reason back into an
// errors.Is-able SDK sentinel while preserving the gRPC status, and leaves
// unknown reasons and non-wavewalletdk errors unchanged. It builds inputs from
// the shared wavewalletrpc reason constants, so it doubles as a wire-contract
// check against the daemon-side mapper.
func TestReconstructSentinel(t *testing.T) {
	...
	for _, tc := range cases {
		in := statusWithReason(
			t, wavewalletrpc.FailureDomain, tc.reason,
		)
	...
	unknown := statusWithReason(
		t, wavewalletrpc.FailureDomain, "SOMETHING_ELSE",
	)
	require.Equal(t, unknown, reconstructSentinel(unknown))

	// A reason from a different domain is ignored.
	foreign := statusWithReason(
		t, "other", wavewalletrpc.ReasonAmountInvalid,
	)
