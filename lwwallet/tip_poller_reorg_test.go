...
// newManualReorgTestPoller creates a poller whose background interval cannot
// race the test's two-step chain mutation. Tests call poll directly after each
// atomic stub-chain update, making the exact snapshot under assertion
// deterministic even under the race detector.
func newManualReorgTestPoller(t *testing.T,
	seedHeight int32) (*TipPoller, *stubChain) {
	...
	srv := mockEsploraServer(t, stubEsploraHandler(t, chain))
	tp := NewTipPoller(
		NewEsploraClient(srv.URL, btclog.Disabled), time.Hour,
		btclog.Disabled,
	)
	require.NoError(t, tp.Start())
	t.Cleanup(tp.Stop)
...
func TestTipPollerDeeperForwardReorg(t *testing.T) {
	...
	tp, chain := newManualReorgTestPoller(t, 100)
...
// TestTipPollerReorgDuringMultiBlockAdvance verifies continuity at every
// height of a catch-up walk. The backend changes chain after height 111 has
// been broadcast but before height 112 is fetched; the poller must disconnect
// old 111 instead of permanently stitching new 112 onto it.
func TestTipPollerReorgDuringMultiBlockAdvance(t *testing.T) {
	t.Parallel()

	chain := newStubChain(110)
	...
	srv := mockEsploraServer(
		t, http.HandlerFunc(func(w http.ResponseWriter,
			...
			if r.URL.Path == "/block-height/112" {
				reorgOnce.Do(func() {
					chain.reorgTo(t, 110, 112, 1)
				})
			}
	...
	chain.advance(t, 2)
	chain.mu.Lock()
	orphaned111 := chain.hashAt[111]
	chain.mu.Unlock()

	// Call the walk directly so the test controls the precise request at
	// which the backend swaps chains.
	tp.advance(110, 112)

	select {
	case ev := <-reorgSub.Updates():
		require.NotNil(t, ev)
		require.Equal(t, int32(110), ev.ForkHeight)
		require.Equal(t, []chainhash.Hash{orphaned111}, ev.Disconnected)
		require.Len(t, ev.Connected, 2)
		require.Equal(t, int32(111), ev.Connected[0].Height)
		require.Equal(t, int32(112), ev.Connected[1].Height)

	case <-time.After(2 * time.Second):
		t.Fatal("timed out waiting for mid-advance reorg event")
	}
