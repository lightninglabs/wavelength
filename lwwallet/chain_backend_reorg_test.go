...
type fakeChain struct {
	...
	// failOutspend marks outpoints whose outspend endpoint should return
	// a transient server error.
	failOutspend map[wire.OutPoint]struct{}
...
func newFakeChain(t *testing.T, tip int32, tag string) *fakeChain {
	...
	c := &fakeChain{
		...
		failOutspend:  make(map[wire.OutPoint]struct{}),
...
// setOutspendFailure controls whether an outpoint lookup returns a transient
// server error.
func (c *fakeChain) setOutspendFailure(op wire.OutPoint, fail bool) {
	c.mu.Lock()
	defer c.mu.Unlock()

	if fail {
		c.failOutspend[op] = struct{}{}

		return
	}

	delete(c.failOutspend, op)
}
...
func (c *fakeChain) serveTxReq(w http.ResponseWriter, _ *http.Request,
	...
	case "outspend":
		...
		if _, fail := c.failOutspend[op]; fail {
			http.Error(
				w, "temporary outspend failure",
				http.StatusInternalServerError,
			)
...
// TestChainBackendSpendReorgSkipsTransientStatusFailure verifies that a
// temporary Esplora failure leaves a positive spend registration untouched.
// Treating an undetermined lookup as unspent would emit a false Reorged.
func TestChainBackendSpendReorgSkipsTransientStatusFailure(t *testing.T) {
	t.Parallel()

	chain := newFakeChain(t, 100, "spend-transient-init")
	srv := fakeChainServer(t, chain)
	...
	outpoint := wire.OutPoint{
		Hash:  chainhash.HashH([]byte("transient-funding")),
		Index: 0,
	}
	chain.setOutspend(outpoint, esploraOutspend{
		Spent: true, Txid: spenderTxid.String(), Vin: 0,
	...
	awaitSpend(t, reg.Spend)
	chain.setOutspendFailure(outpoint, true)

	backend.mu.Lock()
	var (
		id       uint64
		internal *spendRegistration
	)
	for regID, candidate := range backend.spendRegs {
		id = regID
		internal = candidate
		break
	}
	...
	backend.reorgSpendReg(
		id, internal, map[chainhash.Hash]struct{}{},
	)

	select {
	case <-reg.Reorged:
		t.Fatal("transient outspend failure emitted false Reorged")
