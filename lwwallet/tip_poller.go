...
//     fetch each intervening hash; if the first new block's PrevBlock
//     header field does not point at the cached tip hash, walk back
//     to find the fork point, emit a ReorgEvent for the disconnected
//     range, and then fan TipBlock events for the new chain in
//     ascending order. If the new chain extends the old tip cleanly
//     (the common case), no ReorgEvent fires and TipBlock events are
...
// detecting reorgs along the way. The very first new height's raw
// header is consulted to verify its PrevBlock matches the cached
// tip hash; mismatch triggers a walk-back to the fork point and a
// reorg emission before any new TipBlock events fire.
func (t *TipPoller) advance(oldHeight, newHeight int32) {
	...
	for height := oldHeight + 1; height <= newHeight; height++ {
		...
		// On the FIRST new height, verify continuity against
		// the cached tip. We use the raw 80-byte header (which
		// carries PrevBlock) because the JSON header endpoint
		// does not include the previous-block hash. If
		// PrevBlock does not match the cached tip hash a reorg
		// crossed the boundary; walk back to the fork point
		// and emit a ReorgEvent before continuing.
		if height == oldHeight+1 {
			rawHdr, err := t.esplora.GetRawBlockHeader(
				context.Background(), hash,
			)
			if err != nil {
				// Without the raw header we cannot verify
				// that the new block connects to the cached
				// tip. Abort the cycle without broadcasting
				// or updating the cached tip; the next tick
				// will retry. Optimistically advancing here
				// would permanently hide a reorg that
				// crossed the oldHeight boundary if the
				// raw-header fetch flaked exactly when the
				// reorg happened.
				t.log.WarnS(
					context.Background(),
					"Tip poller raw header fetch "+
						"failed; aborting cycle",
					err,
					slog.String("hash", hash.String()),
				)
			...
			t.mu.Lock()
			cachedOldHash, ok := t.hashAtHeightLocked(
				oldHeight,
			)
			t.mu.Unlock()

			if ok && rawHdr.PrevBlock != cachedOldHash {
				// Deeper reorg: handle it as a new-tip
				// reorg from oldHeight, pointing at the
				// actual new tip at newHeight. handleReorg
				// emits ReorgEvent + TipBlock events
				// itself.
				tipHash, tipErr := t.esplora.
