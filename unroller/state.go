...
type UnrollState struct {
	...
	// Re-derived from VTXO.TreePath on restart via extractLevelOrder.
	//
	// TODO(#199): Persist total_levels in the DB as a sanity check.
	// Once cross-batch ancestry is supported, the tree may span
	// multiple commitment txs and the depth at unroll time could
	// differ from a later re-derivation.
...
func extractLevelOrder(treePath *tree.Tree) ([]LevelTxids, error) {
	...
	// maxBFSNodes bounds the traversal to prevent OOM on
	// corrupt or maliciously large trees. A legitimate
	// single-client path has O(log N) nodes.
	const maxBFSNodes = 10_000

	queue := tree.NewQueue[*nodeWithLevel]()
	queue.Enqueue(&nodeWithLevel{node: treePath.Root, level: 0})
