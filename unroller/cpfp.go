...
// minimize change output size and waste. The optional exclude map
// tracks outpoints already selected within the same broadcast level,
// preventing double-spend when multiple nodes are broadcast in
// parallel.
func selectFeeUTXO(ctx context.Context, wk WalletKit,
	minValue btcutil.Amount,
	exclude map[wire.OutPoint]bool) (*feeUTXO, error) {
...
// findAnchorOutput scans a transaction's outputs for the P2A
// ephemeral anchor script and returns its index and value. Returns
// -1 and nil if no anchor is found.
func findAnchorOutput(
	tx *wire.MsgTx,
) (int, *wire.TxOut) {
