...
- `ChainBackend` — Implements `chainsource.ChainBackend` using neutrino's native `ChainNotifier` for confirmation tracking, spend detection, and fee estimation. `SubmitPackage` is implemented but returns an unsupported error (neutrino has no package relay capability).
