...
// TestRegisterLifecycleDefersEarlyDone proves the neutrino forwarders preserve
// positive-before-terminal delivery when both chainntnfs lifecycle fields are
// buffered and a select happens to observe Done first.
func TestRegisterLifecycleDefersEarlyDone(t *testing.T) {
	...
	t.Run("confirmation", func(t *testing.T) {
		...
		backend := &ChainBackend{notifier: notifier}
		reg, err := backend.RegisterConf(
			t.Context(), &chainhash.Hash{0x61}, []byte{0x51}, 1,
			100, false,
		)
		require.NoError(t, err)
