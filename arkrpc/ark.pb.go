...
type GetInfoResponse struct {
	...
	// lnd_node_pubkey is the compressed lnd node key used for integrated
	// virtual Lightning channels. Empty when the operator has no integrated
	// lnd runtime.
	LndNodePubkey []byte `protobuf:"bytes,23,opt,name=lnd_node_pubkey,json=lndNodePubkey,proto3" json:"lnd_node_pubkey,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}
...
func (x *GetInfoResponse) GetLndNodePubkey() []byte {
	if x != nil {
		return x.LndNodePubkey
	}
	return nil
}
...
type VirtualChannelBackingVTXO struct {
	...
	// pk_script is the backing VTXO script. It is required for
	// round-created VTXOs that are not yet present in the operator's
	// durable VTXO store.
	PkScript []byte `protobuf:"bytes,3,opt,name=pk_script,json=pkScript,proto3" json:"pk_script,omitempty"`
	// policy_template is the backing VTXO policy template. It is required
	// for round-created VTXOs so the operator can cosign the cooperative
	// VTXO-to-channel spend before the round is broadcast.
	PolicyTemplate []byte `protobuf:"bytes,4,opt,name=policy_template,json=policyTemplate,proto3" json:"policy_template,omitempty"`
	unknownFields  protoimpl.UnknownFields
	sizeCache      protoimpl.SizeCache
}
...
func (x *VirtualChannelBackingVTXO) GetPkScript() []byte {
	if x != nil {
		return x.PkScript
	}
	return nil
...
func (x *VirtualChannelBackingVTXO) GetPolicyTemplate() []byte {
	if x != nil {
...
type RequestVirtualChannelIntentRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// pending_channel_id is lnd's 32-byte PSBT funding identifier chosen by
	// the client before the acceptor row is persisted.
	PendingChannelId []byte `protobuf:"bytes,1,opt,name=pending_channel_id,json=pendingChannelId,proto3" json:"pending_channel_id,omitempty"`
	// client_node_pubkey is the compressed lnd node key of the client that
	// will accept the operator-opened channel.
...
func (x *RequestVirtualChannelIntentRequest) GetClientNodePubkey() []byte {
	if x != nil {
		return x.ClientNodePubkey
	}
	return nil
...
type RequestVirtualChannelIntentResponse struct {
	...
	SelectedVtxos []*VirtualChannelBackingVTXO `protobuf:"bytes,5,rep,name=selected_vtxos,json=selectedVtxos,proto3" json:"selected_vtxos,omitempty"`
	// backing_tx is the unsigned VTXO-to-channel-point transaction.
	BackingTx []byte `protobuf:"bytes,6,opt,name=backing_tx,json=backingTx,proto3" json:"backing_tx,omitempty"`
	// funding_psbt is the lnd funding PSBT retained for audit/recovery.
...
func (x *RequestVirtualChannelIntentResponse) GetSelectedVtxos() []*VirtualChannelBackingVTXO {
	if x != nil {
...
const file_ark_proto_rawDesc = "" +
	...
	"\x0eSTATE_DISABLED\x10\x02\"\xbe\x06\n" +
	...
	"\x0flnd_node_pubkey\x18\x17 \x01(\fR\rlndNodePubkey\"\x7f\n" +
	...
	"\x12below_dust_warning\x18\a \x01(\bR\x10belowDustWarning\"\x9c\x01\n" +
	...
	"\rchannel_point\x18\x04 \x01(\tR\fchannelPoint\x12H\n" +
