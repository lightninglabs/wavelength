...
// reorgWaitTimeout is the per-step deadline used by the LNDBackend
// forwarder reorg tests. The forwarder is a tight goroutine, so the
// timeout exists only to make a hang surface as a fast failure on slow CI.
const reorgWaitTimeout = 2 * time.Second

// TestRegisterConfForwardsReorgAndDone drives the full confirmation
// lifecycle through the chainntnfs notifier into the LNDBackend forwarder
// and asserts each event arrives on the matching chainsource registration
// channel. The lifecycle is:
//
//	Confirmed -> NegativeConf -> Confirmed -> Done
//
// and the test additionally verifies the forwarder exits after Done by
// observing that the chainsource channels close.
func TestRegisterConfForwardsReorgAndDone(t *testing.T) {
	...
	// this layer; consumers must not rely on it.
	negChan <- 1
	...
	// All three forwarded channels must close once the forwarder exits.
...
// TestRegisterLifecycleDefersEarlyDone proves the backend forwarders preserve
// the required positive-before-terminal contract even when chainntnfs has both
// lifecycle fields buffered and a select observes Done first.
func TestRegisterLifecycleDefersEarlyDone(t *testing.T) {
	t.Parallel()
	...
	t.Run("confirmation", func(t *testing.T) {
		...
		reg, err := backend.RegisterConf(
			t.Context(), &chainhash.Hash{0x51}, []byte{0x51}, 1,
			100, false,
		)
		require.NoError(t, err)

		doneChan <- struct{}{}
		requireNoStruct(t, reg.Done, "confirmation Done")
		hash := chainhash.Hash{0x52}
		confChan <- &chainntnfs.TxConfirmation{
			BlockHash:   &hash,
			BlockHeight: 123,
			Tx:          wire.NewMsgTx(2),
		}
		confirmed := awaitConfForward(t, reg.Confirmed)
		require.Equal(t, hash, *confirmed.BlockHash)
		awaitStruct(t, reg.Done, "deferred confirmation Done")
	})
	...
	t.Run("spend", func(t *testing.T) {
		...
		doneChan <- struct{}{}
		requireNoStruct(t, reg.Done, "spend Done")
		hash := chainhash.Hash{0x53}
		spendChan <- &chainntnfs.SpendDetail{
			SpentOutPoint:  outpoint,
			SpenderTxHash:  &hash,
			SpendingTx:     wire.NewMsgTx(2),
		...
		require.Equal(
			t, hash, *awaitSpendForward(t, reg.Spend).SpenderTxHash,
		)
		awaitStruct(t, reg.Done, "deferred spend Done")
	})
}

// requireNoStruct asserts a lifecycle signal does not arrive during a short
// scheduling window.
func requireNoStruct(t *testing.T, ch <-chan struct{}, label string) {
	t.Helper()

	select {
	case <-ch:
		t.Fatalf("%s arrived before its positive observation", label)

	case <-time.After(50 * time.Millisecond):
	}
}

// awaitConfForward reads a single confirmation off the forwarded channel
...
// awaitSpendForward reads a single spend off the forwarded channel with a
// deadline, failing the test on timeout or unexpected close.
...
// requireStructClosedSoon asserts that a struct{} signal channel closes
// within the reorg wait timeout.
...
// awaitSeq reads a single sequence number off the forwarded Reorged
// channel with a deadline. The reorg signal now carries the forwarder's
// monotonic sequence (see chainsource.TxConfirmation.Seq) rather than a
// bare struct{}.
