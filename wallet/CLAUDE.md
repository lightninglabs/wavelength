...
- `RefreshCustomVTXOsRequest` / `RefreshCustomVTXOsResponse` — Ask-request to queue caller-supplied custom-policy VTXOs (`CustomRefreshInput`) for refresh against caller-specified replacement outputs (`CustomRefreshOutput`); the wallet does not select these from live balance but activates temporary `PendingForfeit` signer actors for the old inputs so the round can later collect the local signature. `DropCustomRefreshVTXOsRequest` / `Response` removes those temporary signers when round registration never starts.
- `PendingIntentStore` / `PendingIntentReplayer` / `PendingIntent` / `PendingIntentPayload` — Restart-safe "pending intents" outbox. The wallet persists a `BoardIntentPayload` or `SendOnChainIntentPayload` (via `NewPendingIntentID`, a deterministic hash of kind + sorted anchor outpoints + payload fields) before publishing to the round actor; a kind-specific `PendingIntentReplayer` (`boardIntentReplayer`, `sendOnChainIntentReplayer`) reconciles persisted rows against live state at startup and self-Tells the original command (`BoardRequest`, `ReplaySendOnChainIntent`) for anchors still live, clearing stale rows otherwise. `ReplayPendingIntentsRequest` / `Response` is the daemon-driven Ask that triggers this after the round-client actor is receptionist-reachable.
- `BoardingSweepStore` / `NewBoardingSweep` / `BoardingSweepRecord` — Persistence interface and records for aggregate boarding-timeout sweeps (batching expired boarding UTXOs into one sweep transaction). `SweepBoardingUTXOsRequest` / `Response` previews or broadcasts a sweep; `ResumeBoardingSweepsRequest` / `Response` re-arms spend watches and re-submits pending sweeps to `txconfirm` at startup.
- `WalletBackingSweeper` / `SweepWalletFundsRequest` / `SweepWalletFundsResponse` — Interface and Ask-request for a general backing-wallet sweep that drains every confirmed (non-boarding) wallet UTXO to a single destination address; distinct from the boarding-timeout sweep above.
...
- **Depends on**: `baselib/actor` (actor system), `chainsource` (block epoch/tip-tick notifications, spend notifications for boarding-sweep inputs, fee estimation), `lib/actormsg` (VTXO manager admission types), `lib/types` (`OperatorTerms`, `LeaveRequest`, `VTXORequest.Origin`, `Ancestry`), `lib/arkscript` (policy template encode/decode, spend paths, VTXO spend info), `lib/tx/arktx` (shared `TxVersion` constant for sweep transactions), `txconfirm` (submit/track boarding-sweep and wallet-sweep broadcasts to confirmation), `walletcore` (`LockID`, `OutputLeaser`, `Utxo` types shared across backends), `ledger` (`Sink` alias for emission + `UTXOCreatedMsg` / `ClassificationDeposit` constants).
- **Depended on by**: `round` (boarding intents, types: `BoardingAddress`, `SelectedVTXO`), `db` (persistence), `darepod` (wiring, RPC handlers), `daemonrpc` (error mapping), `swapwallet` (shared wallet deps), `btcwbackend` / `lndbackend` / `lwwallet` (implement `BoardingBackend` / `OutputLeaser`).
- **Sends**:
...
- **Receives**:
  ...
  - ← API: `CreateBoardingAddressRequest`, `GetActiveBoardingAddressesRequest`, `GetBoardingBalanceRequest`, `GetConfirmedBoardingIntentsRequest`, `RefreshVTXOsRequest`, `RefreshCustomVTXOsRequest`, `DropCustomRefreshVTXOsRequest`, `SelectAndLockVTXOsRequest`, `LeaveVTXOsRequest`, `BoardRequest`, `CompleteSpendVTXOsRequest`, `UnlockVTXOsRequest`, `SendVTXOsRequest`, `SendOnChainRequest`, `SweepBoardingUTXOsRequest`, `SweepWalletFundsRequest`
  - ← `darepod` (startup orchestration, after the round-client actor is receptionist-reachable): `ReplayPendingIntentsRequest`, `ResumeBoardingSweepsRequest`
...
- Boarding into VTXOs is clamped by `applyBoardingLimits`/`clampBoardingAmount` (`board_limits.go`) to the operator's advertised `MaxVTXOAmount` (per-VTXO) and `MaxUserBalance` (total) terms: balance over the cap is capped to headroom (`ErrBoardingCapReached`), a balance below the dust/`MinBoardingAmount` floor is rejected (`ErrBoardAmountBelowFloor`), and a clipped remainder is returned on-chain via a fresh boarding-address change leave rather than minting a sub-dust VTXO. A per-VTXO maximum below the floor is a misconfiguration (`ErrMaxVTXOBelowFloor`); a maximum so small it would require more than `maxBoardOutputs` (1000) pieces is rejected (`ErrTooManyBoardOutputs`) rather than silently wrapping the split count.
- User-issued `Board` and `SendOnChain` requests are persisted to the `PendingIntentStore` before publishing to the round actor (persist-before-publish); the round-state checkpoint clears an intent's anchors in the same SQL transaction that records adoption, so replay-after-adoption is structurally impossible and a restart only ever re-issues intents whose anchors are still live.
