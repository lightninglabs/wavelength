...
- The tip-tick loop (`runTipTickLoop`) exits immediately on `actor.ErrActorTerminated` from `Tell`. The actor system can terminate without calling `Stop()` on individual actors; a dead mailbox will never recover, so re-logging against it is pointless. Other Tell errors are debug-logged and the loop continues.
