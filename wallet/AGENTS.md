...
- `SelectedVTXO` — Describes a VTXO selected and locked for use as a transfer input (outpoint, amount, pkscript, `ReserveEpoch`). Breaks the vtxo → round → wallet import cycle. `handleSelectAndLockVTXOs` copies `ReserveEpoch` straight through from the manager's `actormsg.SelectedVTXO` so the OOR transfer that spends the coin can name the reservation on release; the wallet neither mints nor interprets the value.
