...
- **A refresh intent leaves its signing descriptor empty.** It must not reuse
  the old VTXO's owner descriptor as the round's MuSig2 tree-signing key: a
  legacy descriptor can carry a usable pubkey with no LND key locator, and
  while join authentication can sign by pubkey, MuSig2 session creation needs
  the locator and aborts the round without it. Leaving the descriptor empty
  lets the existing registration path derive a fresh
  `types.VTXOSigningKeyFamily` key. The old owner descriptor still owns the
  replacement VTXO and signs the forfeit input, so ownership is unchanged.
