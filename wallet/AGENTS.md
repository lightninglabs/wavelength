...
- `SelectedVTXO` — Describes a VTXO selected and locked for use as a transfer input (outpoint, amount, pkscript, and the manager's `ReserveEpoch` for the reservation). Breaks the vtxo → round → wallet import cycle.
...
- `handleRefreshVTXOs` leaves `RefreshVTXORequest.SigningKey` **empty** rather than reusing the old VTXO's `ClientKey`. Round registration derives a fresh locator-backed key per refreshed output (`round.ensureVTXOSigningKeys`, which fills only missing keys), and a legacy VTXO can retain the owner pubkey without its LND key locator — reusing that descriptor breaks MuSig2 tree signing. `OwnerKey` still carries the stable owner descriptor; ownership and signing identity are deliberately separate here.
- `SelectedVTXO.ReserveEpoch` is a pass-through, not a wallet-owned value: `handleSelectAndLockVTXOs` copies the epoch the VTXO manager stamped on `SelectAndReserveSpendResponse` so the OOR transfer that spends the coin can name the reservation it held when it releases. Dropping it silently degrades the manager's stale-release guard to unconditional release; see the epoch invariants in [`vtxo/CLAUDE.md`](../vtxo/CLAUDE.md).
