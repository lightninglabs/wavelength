...
// BuildAssetTree builds an asset VTXO tree below a batch output.
func BuildAssetTree(ctx context.Context, cfg TreeMaterializerConfig,
	req AssetTreeRequest) (*tree.Tree, error) {

	return buildAssetTree(ctx, cfg, req, &sdkDriver{
		wallet: cfg.Wallet,
	})
}

func buildAssetTree(ctx context.Context, cfg TreeMaterializerConfig,
	req AssetTreeRequest, driver assetTreeDriver) (*tree.Tree, error) {

	if req.BatchOutput == nil {
		return nil, fmt.Errorf("batch output is required")
	}
	if req.BatchOutput.Value <= 0 {
		return nil, fmt.Errorf("batch output value must be positive")
	}
	if !bytes.Equal(
		req.BatchOutput.PkScript, cfg.Root.BatchPkScript,
	) {
		return nil, fmt.Errorf("batch output script does not match " +
			"the root asset source")
	}
	...
	for idx := range req.Leaves {
		leaf := &req.Leaves[idx]
		if leaf.Amount <= 0 {
			return nil, fmt.Errorf("leaf %d carrier value must be "+
				"positive", idx)
		}
		if leaf.AssetAmount == 0 {
			return nil, fmt.Errorf("leaf %d carries no "+
				"asset amount", idx)
		}
		if leaf.CoSignerKey == nil {
			return nil, fmt.Errorf("leaf %d cosigner is required",
				idx)
		}
		owner := string(leaf.CoSignerKey.SerializeCompressed())
		if _, ok := owners[owner]; ok {
			return nil, fmt.Errorf("leaf %d repeats a cosigner",
				idx)
		}
		owners[owner] = struct{}{}

		if leaf.Amount > btcutil.Amount(math.MaxInt64)-btcTotal {
			return nil, fmt.Errorf("leaf carrier value overflow")
		}
		if leaf.AssetAmount > math.MaxUint64-assetTotal {
			return nil, fmt.Errorf("leaf asset amount overflow")
		}
		btcTotal += leaf.Amount
		assetTotal += leaf.AssetAmount
	}
	if btcTotal != btcutil.Amount(req.BatchOutput.Value) {
		return nil, fmt.Errorf("leaf amounts %d do not consume the "+
			"batch output value %d", btcTotal,
			req.BatchOutput.Value)
	}
	if assetTotal != req.AssetAmount {
		return nil, fmt.Errorf("leaf asset amounts %d do not consume "+
			"the batch asset amount %d", assetTotal,
			req.AssetAmount)
	}
	var rootTotal uint64
	for idx := range cfg.Root.Inputs {
		amount := cfg.Root.Inputs[idx].Amount
		if amount > math.MaxUint64-rootTotal {
			return nil, fmt.Errorf("root asset amount overflow")
		}
		rootTotal += amount
	}
	if rootTotal != req.AssetAmount {
		return nil, fmt.Errorf("root asset inputs carry %d, want %d",
			rootTotal, req.AssetAmount)
	}
	...
	if structure == nil || structure.AssetContext == nil {
		return nil, fmt.Errorf("asset tree structure has no asset " +
			"context")
	}
	if err := validateTreeProofCapacity(
		cfg.Root, structure.Root.Depth(),
	); err != nil {
		return nil, err
	}

	materializer, err := newTreeMaterializer(
		cfg, structure.AssetContext, req.BatchOutpoint, driver,
	)
	if err != nil {
		return nil, err
	}
	structure.AssetContext.SetAssetRef(cfg.AssetRef.String())
	...
	sweepRoot := cfg.SweepLeaf.TapHash()
	built := &tree.Tree{
		Root:               structure.Root,
	...
	if err := built.Verify(); err != nil {
		return nil, fmt.Errorf("materialized tree is inconsistent: %w",
			err)
	}
