...
type TreeMaterializerConfig struct {
	...
	// Store journals every tapd commit attempt before it can mutate asset
	// state and retains the sealed package used for deterministic replay.
	Store Store
...
func NewTreeMaterializer(cfg TreeMaterializerConfig) (*TreeMaterializer,
	...
	hasProofFile := len(cfg.Root.ProofFile) != 0
	hasProofPath := len(cfg.Root.ProofPath) != 0
	if cfg.Root.proofPath == nil && hasProofFile == hasProofPath {
		return nil, fmt.Errorf("exactly one root proof source is " +
			"required")
	}
...
func (m *TreeMaterializer) commitNode(ctx context.Context, node *tree.Node,
	...
	return m.commitDurably(ctx, input, request, source.verifier)
}
...
func (m *TreeMaterializer) validateCommit(input wire.OutPoint, amount uint64,
	...
	if !committed.inputs[0].assetRef.Equivalent(m.cfg.AssetRef) {
		return nil, fmt.Errorf("asset input ref mismatch")
	}
	...
	if committed.inputs[0].amount != expectedTotal {
		return nil, fmt.Errorf("asset input amount %d, want %d",
			committed.inputs[0].amount, expectedTotal)
	}
	...
	for _, spec := range specs {
		...
		if !out.assetRef.Equivalent(m.cfg.AssetRef) {
			return nil, fmt.Errorf("output %d asset ref mismatch",
				spec.index)
		}
		...
		if out.anchorOutpoint != expectedOutpoint {
			return nil, fmt.Errorf("output %d anchor outpoint "+
				"mismatch", spec.index)
		}
