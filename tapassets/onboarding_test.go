...
// TestOnboarderRejectsPassiveAssets prevents silently moving
...
func TestOnboarderRejectsPassiveAssets(t *testing.T) {
	...
	_, err = onboarder.Onboard(t.Context(), request)
	require.ErrorContains(t, err, "proof for every asset")
...
func TestOnboarderRejectsDuplicateFundingProof(t *testing.T) {
	...
	_, err = onboarder.Onboard(t.Context(), request)
	require.ErrorContains(t, err, "selects asset record")
	require.Zero(t, driver.commits)
}
...
func (d *fakeOnboardingDriver) CommitOnboarding(ctx context.Context,
	...
	result, err := fakeOnboardingCommit(ctx, request, verifier)
	if err != nil {
		return nil, err
	}
	...
	for idx := range request.Inputs {
		result.inputs[idx] = commitInput{
			...
			anchorOutpoint: sdkOutpoint(
				template.UnsignedTx.TxIn[min(
					idx, len(template.UnsignedTx.TxIn)-1,
				)].PreviousOutPoint,
			),
...
func (d *fakeOnboardingDriver) Publish(_ context.Context, packageBytes,
	finalPSBT []byte) error {
...
func testOnboardingRequest(t *testing.T) (*OnboardingRequest, *fakeInventory,
	...
	t.Helper()
	issuance := tapsdk.AssetID(sha256Bytes([]byte("asset-id")))
	assetRef := tapsdk.AssetRefFromAssetID(issuance)

	scriptKey, err := tapsdk.ParsePubKey(
		testPrivateKey(t, 4).PubKey().SerializeCompressed(),
	)
	require.NoError(t, err)
	anchorOutpoint := tapsdk.Outpoint{
		Txid:  sha256Bytes([]byte("input-outpoint")),
		Index: 1,
	}
	asset := &tapsdk.AssetRecord{
		...
		Amount: 21, ScriptKey: tapsdk.ScriptKey{
			PubKey: scriptKey,
		},
	}
	inventory := &fakeInventory{
		verification: &tapsdk.VerifyProofResponse{
			...
			DecodedProof: &tapsdk.DecodedProof{
				AssetRef: assetRef, IssuanceID: issuance,
				ScriptKey: scriptKey, Amount: 21,
...
// TestOnboarderSharedAnchor consumes all same-issuance records with one
// Bitcoin input and signing plan, including after a journal replay.
func TestOnboarderSharedAnchor(t *testing.T) {
	t.Parallel()
	req, inventory, owner := testMultiUtxoOnboardingRequest(t, 40)
	firstProof := inventory.verifications[string(req.ProofFiles[0])]
	firstTip := firstProof.DecodedProof
	secondProof := inventory.verifications[string(req.ProofFiles[1])]
	secondTip := secondProof.DecodedProof
	first := inventory.utxos[firstTip.Outpoint.String()]
	second := inventory.utxos[secondTip.Outpoint.String()]
	delete(inventory.utxos, secondTip.Outpoint.String())
	secondTip.Outpoint = firstTip.Outpoint
	first.Assets = append(first.Assets, second.Assets...)
	driver := newFakeOnboardingDriver()
	store, err := NewFileStore(t.TempDir())
	require.NoError(t, err)
	onboarder := testOnboarder(driver, inventory, store, owner)
	result, err := onboarder.Onboard(t.Context(), req)
	require.NoError(t, err)
	require.EqualValues(t, 40, result.AssetAmount)
	dto := driver.requests[0]
	require.Len(t, dto.Inputs, 2)
	require.Len(t, dto.SigningPlans, 1)
	anchor, err := psbtutil.Parse(dto.AnchorPSBT)
	require.NoError(t, err)
	require.Len(t, anchor.UnsignedTx.TxIn, 1)
	replay, err := onboarder.Onboard(t.Context(), req)
	require.NoError(t, err)
	require.Equal(t, result.Outpoint, replay.Outpoint)
	require.Equal(t, 1, driver.commits)
}

// TestOnboarderRejectsMultipleIssuances rejects an unsupported grouped
// selection before a commit could create multiple boarding proof tips.
func TestOnboarderRejectsMultipleIssuances(t *testing.T) {
	t.Parallel()
	req, inventory, owner := testMultiUtxoOnboardingRequest(t, 40)
	secondProof := inventory.verifications[string(req.ProofFiles[1])]
	second := secondProof.DecodedProof
	second.IssuanceID[0] ^= 1
	driver := newFakeOnboardingDriver()
	store, err := NewFileStore(t.TempDir())
	require.NoError(t, err)
	_, err = testOnboarder(driver, inventory, store, owner).Onboard(
		t.Context(), req,
	)
	require.ErrorContains(t, err, "one issuance")
	require.Zero(t, driver.commits)
}

// ReconcileOnboarding models read-only backend evidence after a lost response.
func (d *fakeOnboardingDriver) ReconcileOnboarding(context.Context,
	[]byte) error {
...
// TestOnboarderReconcilesPublication never republishes an ambiguous attempt.
func TestOnboarderReconcilesPublication(t *testing.T) {
	t.Parallel()
	request, inventory, owner := testOnboardingRequest(t)
	driver := newFakeOnboardingDriver()
	driver.publishErr = &tapsdk.CustomAnchorPublishAttemptError{
		Err:            errors.New("response lost"),
		OutcomeUnknown: true,
	}
	driver.reconcileErr = ErrReconciliationRequired
	store, err := NewFileStore(t.TempDir())
	require.NoError(t, err)
	onboarder := testOnboarder(driver, inventory, store, owner)
	_, err = onboarder.Onboard(t.Context(), request)
	...
	require.Equal(t, 1, driver.publishes)
	require.Equal(t, 1, driver.commits)
	require.Equal(t, 2, driver.reconciliations)
}
