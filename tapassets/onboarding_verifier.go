...
// onboardingRecord distinguishes assets sharing one Bitcoin anchor.
...
type proofInventoryVerifier struct {
	scriptKey tapsdk.PubKey
	selected  map[onboardingRecord]struct{}
...
func (v *proofInventoryVerifier) VerifyConfirmedProof(ctx context.Context,
	...
	if !tip.AssetRef.Equivalent(v.assetRef) || tip.Amount != v.amount ||
		tip.Outpoint != v.anchor || tip.ScriptKey != v.scriptKey {
	...
	for _, asset := range anchor.Assets {
		if asset == nil {
			return nil, fmt.Errorf("tapd anchor inventory is " +
				"incomplete")
		}
		id := onboardingRecord{
			anchor:    v.anchor,
		...
		if _, ok := v.selected[id]; !ok {
			return nil, fmt.Errorf("onboarding funding selection " +
				"omits an asset record")
		}
