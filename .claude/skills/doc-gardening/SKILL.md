...
If `$ARGUMENTS` is a specific package path (e.g., `rounds` or `internal/testutils`),
...
find . -name '*.go' -not -path './.git/*' -not -path './vendor/*' \
  -not -path './tools/*' -not -path './client/*' | sed 's|/[^/]*$||' | sort -u | while read dir; do
...
5. **Actor message flows** — what messages does this package send to or receive
   from other subsystems? Look for `Tell()`, `Ask()`, `Send()` calls and
   message type definitions. Document these in the "Messages to/from" section.
...
**Critical**: The "Messages to/from" section under Relationships must capture
inter-subsystem communication. For actor-based packages (rounds, oor,
batchwatcher, batchsweeper, indexer), document:
- What message types are sent and to which package/actor.
- What message types are received and from which package/actor.
- The direction of data flow (requests vs events vs notifications).
...
- Skip generated code directories (`adminrpc`) unless proto definitions changed.
