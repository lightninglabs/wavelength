...
- **Messages to/from**: Only include if the package participates in the actor
  system (sends/receives messages via Tell/Ask). Be specific about message
  type names and direction. For example:
  - "Sends `JoinRoundRequest` -> `clientconn` (to operator)"
  - "Receives `BatchConfirmed` <- `batchwatcher`"
