...
type State struct {
	// ConfirmedTxids lists proof txids the caller has observed confirmed.
	ConfirmedTxids []chainhash.Hash

	// InFlightTxids lists proof txids currently being materialized by the
	// caller but not yet observed confirmed.
	InFlightTxids []chainhash.Hash

	// TargetConfirmHeight records the confirmation height of the target tx.
...
type SweepState struct {
	// Status records whether the sweep is pending, broadcasted, or confirmed.
	Status SweepStatus
...
func (s *State) Validate(proof *recovery.Proof) error {
	...
	for txid := range confirmed {
		if _, ok := proof.Node(txid); !ok {
			return fmt.Errorf("confirmed txid %s is not in proof", txid)
		}
	}

	for txid := range inflight {
		if _, ok := proof.Node(txid); !ok {
			return fmt.Errorf("in-flight txid %s is not in proof", txid)
		}
	}
	...
	if s.TargetConfirmHeight != nil && !confirmedTxidSetContains(
		...
		return fmt.Errorf("target confirm height set without confirmed target")
	}
	...
	for txid := range confirmed {
		if err := ensureParentsConfirmed(proof, confirmed, txid); err != nil {
			return err
		}
	}
...
type Snapshot struct {
	...
	// TargetConfirmed is true once the target transaction itself is confirmed.
...
type CSVInfo struct {
	// TargetConfirmHeight is the block height at which the target confirmed.
	TargetConfirmHeight int32
...
func validateSweepState(sweep SweepState, confirmed map[chainhash.Hash]struct{},
	...
	case SweepStatusPending:
		...
		if sweep.ConfirmHeight != nil {
			return fmt.Errorf("pending sweep must not have a confirm height")
		}
	...
	case SweepStatusBroadcasted:
		...
		if sweep.ConfirmHeight != nil {
			return fmt.Errorf("broadcasted sweep must not have a confirm height")
		}
	...
	case SweepStatusConfirmed:
		...
		if sweep.ConfirmHeight == nil {
			return fmt.Errorf("confirmed sweep must have a confirm height")
		}

		targetTxid := proof.TargetOutpoint().Hash
		if _, ok := confirmed[targetTxid]; !ok {
			return fmt.Errorf("confirmed sweep requires confirmed target")
		}
...
func ensureDisjoint(a, b map[chainhash.Hash]struct{}) error {
	for txid := range a {
		if _, ok := b[txid]; ok {
			return fmt.Errorf("txid %s cannot be both confirmed and in-flight",
				txid)
		}
	}
