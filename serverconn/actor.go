...
func (a *ServerConnectionActor) StartIngress(
	...
	// Prime server-side mailbox registration before the first Pull. The
	// heartbeat envelope carries the same Schnorr and TLS-binding headers
	// as normal outbound traffic, so the server can record the binding
	// without waiting for the first ticker or user request.
