...
- `SendClientEventRequest.cachedCorrelationKey` — Internal field (TLV record type 8, odd = forward-compatible) that caches the per-key FIFO claim lane value at `Encode` time. Required because the outbox-CDC decode path replaces the typed inner `Message` with a `rawServerMessage` that no longer satisfies the `CorrelationKey` interface, so a round-tripped wrapper still delivers the correct FIFO key when first enqueued into the serverconn durable mailbox after crash recovery.
