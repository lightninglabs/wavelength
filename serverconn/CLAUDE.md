...
- **Folded dispatch (transactional path)**: when the delivery store implements `actor.TxAwareDeliveryStore`, the ingress loop delivers in-memory waiter responses BEFORE the write transaction (pre-transaction, at-most-once, no lock) and folds the durable dispatches plus the advanced AckState checkpoint into ONE commit via `runFoldedDispatch`. A batch of k durable envelopes costs one commit instead of k+1; the cursor can never run ahead of the enqueues because a failure rolls back both. The split-time waiter check (`hasResponseWaiter`) is a hint only — waiters that vanish between the peek and delivery become stragglers and fold into the durable batch so their enqueue commits inside the cursor fold. The legacy (non-transactional) path is unchanged.
