...
- `NewAuthenticatedMailboxClient` / `MailboxAuthSigner`
  (`mailbox_auth_rpc.go`) — `mailboxpb.MailboxServiceClient` decorator that
  signs and attaches the `x-mailbox-auth-sig` header to `Send`, `Pull`, AND
  `AckUpTo` before forwarding to the wrapped edge transport. The operator
  authorizes a mailbox RPC either from the TLS client certificate bound to
  the caller's mailbox ID or from this header, and an operator terminating
  TLS at a proxy never sees a client cert — so the decorator signs
  unconditionally rather than making the operator's TLS posture a client
  config knob. `MailboxAuthSigner` is `func(ctx, recipientMailboxID)
  (hexSig, error)`; it is called per RPC, so implementations are expected
  to memoize (see `waved.Server.mailboxAuthSig`).
...
- **The mailbox auth digest is not uniformly operator-bound.** Only `Send`
  passes the compound `operator:client` recipient, which embeds the
  operator's pubkey-derived ID, so a `Send` signature is useless at any
  other operator. `Pull` and `AckUpTo` pass the client's own plain mailbox
  ID (`ConnectorConfig.LocalMailboxID`, via `ingress.go`), which carries no
  operator component — that digest is identical at every operator, so one
  Pull/Ack signature authorizes that client's mailbox at every operator its
  identity key is known to. Identity keys are a deterministic derivation, so
  a wallet driving two operators presents the same credential to both. Do
  NOT read `MailboxAuthMessage` as preventing cross-server replay on its
  own; closing the Pull/Ack case means folding a server identity into the
  digest, which is a wire change on both sides.
