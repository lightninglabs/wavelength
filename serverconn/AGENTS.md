...
- `SendClientEventRequest` — TLV-durable wrapper for client FSM outbox messages. Carries a `cachedCorrelationKey` field (TLV type 8) persisted alongside Service/Method so that the per-key FIFO ordering key survives an encode/decode cycle through the outbox CDC table. `CorrelationKey()` first asserts the inner `Message` interface; on a decoded wrapper (where `Message` is a `rawServerMessage` that no longer implements the interface), it falls back to the cached bytes. Callers that add new durable client event types must ensure their inner message implements `CorrelationKey() string` so the key is stamped at encode time before the CDC path can observe it.
...
- `SendClientEventRequest.cachedCorrelationKey` is stamped during `Encode` by calling `CorrelationKey()` on the wrapper (which delegates to the inner `Message` on the write path). On the CDC-replay read path, `Decode` populates the cached field from TLV type 8, and `CorrelationKey()` returns it as a fallback. This two-step preserves per-key FIFO claim ordering on the durable mailbox through outbox CDC delivery.
