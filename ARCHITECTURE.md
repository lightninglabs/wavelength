...
| [`sdk/walletdk`](sdk/walletdk/) | Wallet-shaped SDK facade for host apps: `Start` embeds the daemon in-process over a private bufconn transport; `Connect` dials an external daemon over gRPC or REST (`rpc/restclient`). Exposes typed methods for the seven core wallet verbs (create, unlock, send, recv, list, balance, exit). The highest-level layer in the stack; wraps `walletdkrpc.WalletService`. Wallet RPC methods gated behind `walletdkrpc` (which transitively requires `swapruntime`) |
