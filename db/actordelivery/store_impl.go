...
// The LeaseToken is taken from the row as-is, so a peeked (leaseless) row maps
// to an empty token, which is the signal the consume path uses to route
// ack/nack through the unfenced by-ID operations.
	...
	*actor.LeasedMessage, error) {
	...
	err := s.db.ExecTx(ctx, readTxOpts,
		func(q ActorDeliveryQueries) error {
			...
			result = leasedMessageFromRow(msg)
...
func (s *TxActorDeliveryStore) PeekNextMessage(ctx context.Context,
	...
	return leasedMessageFromRow(msg), nil
}
