...
func openSQLiteDatabase(cfg SQLiteOpenConfig) (*SQLiteOpenResult, error) {
	...
	// single-connection handle: it is what makes WAL reachable at all here.
	// Neither wasm VFS implements xShmMap, so the only WAL available is the
	// mode SQLite documents for hosts without shared memory, where an
	// EXCLUSIVE connection keeps the WAL index on the heap. The driver
	// hoists the locking mode ahead of the journal mode for exactly that
	// reason, and then reads back the mode it ended up in, so a regression
	// here surfaces as a startup error rather than as a database quietly
	// running on the rollback journal while `synchronous` was chosen for
	// WAL.
