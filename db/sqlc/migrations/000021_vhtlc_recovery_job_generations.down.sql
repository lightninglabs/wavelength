...
-- The old schema can only store one recovery job for each swap/action. Keep
-- the newest generation and drop older generations during downgrade.
SELECT
    candidate.id, candidate.request_id, candidate.swap_id,
    candidate.direction, candidate.action, candidate.state,
    candidate.vtxo_txid, candidate.vtxo_vout, candidate.vtxo_amount_sat,
    candidate.sender_pubkey, candidate.receiver_pubkey,
    candidate.server_pubkey, candidate.refund_locktime,
    candidate.unilateral_claim_delay, candidate.unilateral_refund_delay,
    candidate.unilateral_refund_without_receiver_delay,
    candidate.preimage_hash, candidate.claim_preimage,
    candidate.signer_key_family, candidate.signer_key_index,
    candidate.destination_script, candidate.max_fee_rate_sat_per_kw,
    candidate.unroll_target_outpoint_hash,
    candidate.unroll_target_outpoint_index, candidate.exit_policy_kind,
    candidate.exit_tx, candidate.exit_txid, candidate.cooperative_txid,
    candidate.last_error, candidate.cancel_reason, candidate.created_at,
    candidate.updated_at, candidate.armed_at, candidate.escalated_at,
    candidate.target_detected_at, candidate.exit_tx_built_at,
    candidate.exit_tx_broadcast_at, candidate.terminal_at
FROM vhtlc_recovery_jobs_old candidate
WHERE NOT EXISTS (
    SELECT 1
    FROM vhtlc_recovery_jobs_old newer
    WHERE newer.swap_id = candidate.swap_id
        AND newer.action = candidate.action
        AND (
