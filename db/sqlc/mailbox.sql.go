...
const CleanupCompletedOutbox = `-- name: CleanupCompletedOutbox :exec
DELETE FROM outbox_messages
WHERE status = 'completed' AND completed_at < $1
`

// Delete completed outbox messages older than a threshold.
func (q *Queries) CleanupCompletedOutbox(ctx context.Context, completedAt sql.NullInt32) error {
	_, err := q.db.ExecContext(ctx, CleanupCompletedOutbox, completedAt)
	return err
}
...
func (q *Queries) DeleteOutboxMessage(ctx context.Context, id string) error {
	_, err := q.db.ExecContext(ctx, DeleteOutboxMessage, id)
	return err
}
