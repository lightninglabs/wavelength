...
func insertRow(ctx context.Context, q *testQueryCreator, id int,
	val string) error {
	...
	_, err := q.tx.ExecContext(ctx, query, id, val)

	return err
}

// TestExecTx_JoinOuterActorTx verifies that when the context carries an outer
...
func TestExecTx_JoinOuterActorTx(t *testing.T) {
	t.Parallel()
	...
	err = txExec.ExecTx(
		ctx, WriteTxOption(), func(q *testQueryCreator) error {
			return insertRow(ctx, q, 1, "hello")
		},
	)
	require.NoError(t, err)
...
// TestExecTx_StandaloneNoTx verifies that when no outer transaction is present
...
func TestExecTx_StandaloneNoTx(t *testing.T) {
	t.Parallel()
	...
	err := txExec.ExecTx(
		...
		func(q *testQueryCreator) error {
			return insertRow(ctx, q, 1, "standalone")
		},
	)
	require.NoError(t, err)
...
// TestExecTx_OuterTxRollbackAtomicity verifies that when two ExecTx calls
...
func TestExecTx_OuterTxRollbackAtomicity(t *testing.T) {
	t.Parallel()
	...
	err = txExec.ExecTx(
		ctx, WriteTxOption(), func(q *testQueryCreator) error {
			return insertRow(ctx, q, 1, "first-store")
		},
	)
	require.NoError(t, err)
	...
	err = txExec.ExecTx(
		ctx, WriteTxOption(), func(q *testQueryCreator) error {
			return insertRow(ctx, q, 2, "second-store")
		},
	)
	require.NoError(t, err)
...
// TestExecTx_OuterTxErrorPropagation verifies that when the txBody returns an
...
func TestExecTx_OuterTxErrorPropagation(t *testing.T) {
	t.Parallel()
	...
	err = txExec.ExecTx(
		ctx, WriteTxOption(), func(q *testQueryCreator) error {
			return insertRow(ctx, q, 1, "success")
		},
	)
	require.NoError(t, err)
