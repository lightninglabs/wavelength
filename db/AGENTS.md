...
- Latest migration: `000009_vtxo_ancestry_paths` drops `tree_path` / `tree_depth` scalar columns from `vtxos` and replaces them with a normalized `vtxo_ancestry_paths` side table (one row per VTXO ancestry fragment). Keyed by `(vtxo_outpoint_hash, vtxo_outpoint_index, path_order)`; FK `ON DELETE CASCADE` keeps the side table consistent. Routine queries (`ListUnspentVTXOs`, `GetVTXO`) skip the ancestry join; the unroller loads ancestry only when resolving an exit. A `UNIQUE(vtxo_outpoint_hash, vtxo_outpoint_index, commitment_txid)` constraint prevents duplicate fragments at the schema level.
