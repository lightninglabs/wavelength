...
- `domainVTXOToInsertParams` — Converts `round.ClientVTXO` to sqlc insert params. Now reads `CommitmentTxID`, `BatchExpiry`, and `CreatedHeight` directly from the domain type (populated at confirmation time by the round FSM), eliminating the two-phase insert pattern where these fields were zeroed on first write.
- `dbVTXOToDomainVTXO` — Converts a sqlc VTXO row back to `round.ClientVTXO`. Parses `CommitmentTxid` blob into `chainhash.Hash` (tolerates short/empty blobs for historical rows by leaving the hash as zero).
- `TransactionExecutor.ExecTx` — Logs each failure point (`Transaction begin/body/commit failed`, `Transaction retries exhausted`) at `WarnS` level so durable-actor DB failures appear in structured logs without the caller needing to add their own retry diagnostics.
