...
const (
	// vtxoCursorNamespace is the namespace used for VTXO event feed cursors.
	vtxoCursorNamespace = "vtxo_events"

	// oorCursorNamespace is the namespace used for OOR recipient event cursors.
	oorCursorNamespace = "oor_recipient_events"
)

// SyncBackend captures the high-level indexer client methods used by SyncClient.
//
// *Client satisfies this interface.
type SyncBackend interface {
	// ListVTXOEventsByScriptsTaproot returns script-scoped VTXO lifecycle events.
	...
	// ListOORRecipientEventsByScriptTaproot returns script-scoped OOR recipient
	// events.
	ListOORRecipientEventsByScriptTaproot(ctx context.Context,
		pkScript []byte, signingKey *btcec.PrivateKey,
		afterEventID uint64, limit uint32, opts ...mailboxrpc.RPCOptions) (
...
type SyncCursorStore interface {
	// LoadCursor returns the cursor for (namespace, key). Unknown keys should
	// return 0, nil.
	LoadCursor(ctx context.Context, namespace string, key string) (
		uint64, error)

	// SaveCursor persists cursor for (namespace, key). Implementations should
	// treat cursors as monotonic and avoid backwards movement.
