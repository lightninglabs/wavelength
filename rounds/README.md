...
## Operator Dispatch (Mailbox RPC Layer)

The `RoundOperator` (`operator.go`) is the adapter between the mailbox
transport layer and the rounds actor. It implements `RoundServiceMailboxServer`
(generated by `protoc-gen-mailboxrpc`) and provides `EnvelopeDispatcher`
closures for the per-client ingress loop.

### Request Flow

```
Client envelope → clientconn ingress → DispatcherMap[{service, method}]
  → makeDispatcher (validates, injects ClientID into ctx)
  → ServeMux.ServeRPC (deserializes proto)
  → Typed handler (e.g. SubmitNonces)
  → Proto→domain conversion (noncesFromProto, etc.)
  → Actor Tell/Ask
  → Response envelope → Edge.Send → Client mailbox
```
...
| `boardingRequestFromProto` | Converts `roundpb.BoardingRequest` to `types.BoardingRequest` |
| `vtxoRequestFromProto` | Converts `roundpb.VTXORequest` to `types.VTXORequest` |
| `parseRoundID` | Converts hex string round ID to `RoundID` |
...
The operator is created in `server_rounds.go:setupRoundsSubsystem` and its
dispatchers are merged with other operators in
