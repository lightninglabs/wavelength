...
// monitoring.
func NewSpendActor(backend ChainBackend, parentCtx context.Context) *SpendActor {
...
func (a *SpendActor) monitorSpend() {
	...
	for {
		...
		case spend, ok := <-a.registration.Spend:
			...
			// In Future mode, exit after first event. In Actor mode,
			// continue monitoring for re-org events.
			if a.promise.IsSome() {
				return
			}
...
// buildSpendEvent converts the backend SpendDetail into a SpendEvent, filling
// in missing fields where possible.
func buildSpendEvent(spend *SpendDetail, watch *SpendActor) (SpendEvent, error) {
