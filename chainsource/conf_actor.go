...
func (a *ConfActor) monitorConfirmation() {
	...
	blockRegCh := make(chan *BlockRegistration)
	var arming bool
	...
	for {
		...
		case reg := <-blockRegCh:
			// Finality arming completed. Clear the flag so a later
			// confirmation can retry if this attempt failed
			// (reg == nil); otherwise wire up the epoch channel.
			arming = false
			if reg != nil {
				a.blockReg = reg
		...
		case seq, ok := <-a.registration.Reorged:
			...
			// The previous confirmation is no longer on the
			// canonical chain. Clear the cached event so a later
			// Done cannot report the reorged-out txid, and reset
			// the depth counter so the next re-confirmation starts
			// a fresh window.
			lastEvent = nil
			a.confirmHeight = 0
...
func (a *ConfActor) armFinalityAsync(regCh chan<- *BlockRegistration,
	log btclog.Logger) {
