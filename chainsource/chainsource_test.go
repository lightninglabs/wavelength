...
func TestChainSourceActorFeeEstimate(t *testing.T) {
	...
	feeResp := resp.(*FeeEstimateResponse)
...
func TestChainSourceActorTestMempoolAccept(t *testing.T) {
	...
	resp, err := result.Unpack()
	require.NoError(t, err)
	acceptResp := resp.(*TestMempoolAcceptResponse)
...
func TestChainSourceActorBroadcastTx(t *testing.T) {
	...
	chainSource := NewChainSourceActor(backend, system)
	ref := ChainSourceKey.Spawn(system, "chainsource-broadcast", chainSource)
	...
	resp, err := result.Unpack()
	require.NoError(t, err)
	broadcastResp := resp.(*BroadcastTxResponse)
...
func TestChainSourceActorBestHeight(t *testing.T) {
	...
	resp, err := result.Unpack()
	require.NoError(t, err)
	heightResp := resp.(*BestHeightResponse)
...
func TestChainSourceActorRegisterConf(t *testing.T) {
	...
	resp, err := result.Unpack()
	require.NoError(t, err)
	confResp := resp.(*RegisterConfResponse)
...
func TestChainSourceActorRegisterSpend(t *testing.T) {
	...
	resp, err := result.Unpack()
	require.NoError(t, err)
	spendResp := resp.(*RegisterSpendResponse)
...
func TestChainSourceActorUnregisterConf(t *testing.T) {
	...
	chainSource := NewChainSourceActor(backend, system)
	ref := ChainSourceKey.Spawn(system, "chainsource-unreg-conf", chainSource)
...
func TestChainSourceActorUnregisterSpend(t *testing.T) {
	...
	chainSource := NewChainSourceActor(backend, system)
	ref := ChainSourceKey.Spawn(system, "chainsource-unreg-spend", chainSource)
...
func TestChainSourceActorUnsubscribeBlocks(t *testing.T) {
	...
	chainSource := NewChainSourceActor(backend, system)
	ref := ChainSourceKey.Spawn(system, "chainsource-unsub-blocks", chainSource)
...
func TestChainSourceActorSubscribeBlocks(t *testing.T) {
	...
	resp, err := result.Unpack()
	require.NoError(t, err)
	epochResp := resp.(*SubscribeBlocksResponse)
...
func TestConfActorNotifyMode(t *testing.T) {
	...
	txHash := chainhash.Hash{}
	notifier := newNotifyActor[ConfirmationEvent]()
	...
	result := confActor.Receive(ctx, &RegisterConfRequest{
		...
		PkScript:    []byte{0x00, 0x14},
		TargetConfs: 1,
		NotifyActor: fn.Some[actor.TellOnlyRef[ConfirmationEvent]](notifier),
	})
	...
	event := notifier.awaitEvent(t, 5*time.Second)
	require.Equal(t, int32(101), event.BlockHeight)
	require.Equal(t, blockHash, event.BlockHash)
}
...
func TestSpendActorNotifyMode(t *testing.T) {
	...
	txHash := chainhash.Hash{}
	outpoint := wire.OutPoint{Hash: txHash, Index: 0}
	notifier := newNotifyActor[SpendEvent]()
...
func TestBlockEpochActorNotifyMode(t *testing.T) {
	...
	notifier := newNotifyActor[BlockEpoch]()
...
func TestBlockEpochActorIteratorCancel(t *testing.T) {
	...
	resp, err := result.Unpack()
	require.NoError(t, err)
	subResp := resp.(*SubscribeBlocksResponse)
	...
	require.NoError(t, err)
	subResp2 := resp2.(*SubscribeBlocksResponse)
...
func TestOutpointOrScriptKey(t *testing.T) {
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			key, err := outpointOrScriptKey(tt.outpoint, tt.pkScript)
...
// notifyActor is a generic helper for testing actors in notify mode. It
// implements TellOnlyRef[T] and captures notifications in a channel for test
// assertions. This can be used for any event type (ConfirmationEvent,
// SpendEvent, BlockEpoch).
type notifyActor[T any] struct {
	events chan T
}

// newNotifyActor creates a new generic notification receiver.
func newNotifyActor[T any]() *notifyActor[T] {
	return &notifyActor[T]{
		events: make(chan T, 10),
	}
}

// ID implements actor.TellOnlyRef[T].
func (n *notifyActor[T]) ID() string {
	return "test-notify-actor"
}

// Tell implements actor.TellOnlyRef[T].
func (n *notifyActor[T]) Tell(ctx context.Context, msg T) {
	select {
	case n.events <- msg:
	case <-ctx.Done():
	}
...
// awaitEvent waits for an event with a timeout.
func (n *notifyActor[T]) awaitEvent(t *testing.T, timeout time.Duration) T {
	t.Helper()
