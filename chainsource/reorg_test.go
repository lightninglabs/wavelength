...
// TestConfActorDefersEarlyDoneUntilConfirmation proves a terminal signal on
// the backend's independent buffered channel cannot overtake the positive
// observation carrying the transaction and confirming-block identity.
func TestConfActorDefersEarlyDoneUntilConfirmation(t *testing.T) {
	t.Parallel()
	...
	result := confActor.Receive(t.Context(), &RegisterConfRequest{
		CallerID:    "test-conf-early-done",
		Txid:        &txHash,
		PkScript:    []byte{0x00, 0x14},
		TargetConfs: 1,
		NotifyActor: fn.Some(confRef),
	...
	require.False(t, ok, "Done overtook the positive observation")

	blockHash := chainhash.Hash{0x32}
	backend.confChan <- &TxConfirmation{
		BlockHash:   &blockHash,
		BlockHeight: 100,
		Tx:          wire.NewMsgTx(2),
	}
	confirmed, ok := confNotifier.AwaitMessage(awaitTimeout)
	require.True(t, ok, "timeout waiting for ConfirmationEvent")
	require.Equal(t, blockHash, confirmed.BlockHash)
	done, ok := doneNotifier.AwaitMessage(awaitTimeout)
	require.True(t, ok, "timeout waiting for deferred ConfDoneEvent")
	require.Equal(t, txHash, done.Txid)
}
...
// TestConfActorRearmsFinalityAfterEpochStreamClose verifies that losing the
// block-epoch stream does not strand a live confirmation below finality. The
// actor must cancel the dead registration, acquire a replacement, and finish
// once the replacement stream reaches the configured safety depth.
func TestConfActorRearmsFinalityAfterEpochStreamClose(t *testing.T) {
	t.Parallel()
	...
	result := confActor.Receive(t.Context(), &RegisterConfRequest{
		CallerID:    "test-conf-finality-rearm",
		Txid:        &txHash,
		PkScript:    []byte{0x51},
		TargetConfs: 1,
		NotifyActor: fn.Some[actor.TellOnlyRef[ConfirmationEvent]](
			confNotifier,
		),
		NotifyReorged: fn.Some[actor.TellOnlyRef[ConfReorgedEvent]](
			reorgNotifier,
		),
		NotifyDone: fn.Some[actor.TellOnlyRef[ConfDoneEvent]](
			doneNotifier,
		),
	})
	require.True(t, result.IsOk())
	...
	first := backend.nextRegistration(t)
	close(first.epochs)
	first.assertCanceled(t)

	second := backend.nextRegistration(t)
	second.send(105)

	done, ok := doneNotifier.AwaitMessage(awaitTimeout)
	require.True(t, ok, "finality did not recover after epoch close")
	require.Equal(t, txHash, done.Txid)
}
...
// TestSpendActorDefersEarlyDoneUntilSpend is the spend-side proof that Done
// cannot erase the spender identity required to classify an own spend versus
// a conflicting double spend.
func TestSpendActorDefersEarlyDoneUntilSpend(t *testing.T) {
	t.Parallel()
	...
	outpoint := wire.OutPoint{Hash: chainhash.Hash{0x41}, Index: 1}
	spendNotifier := actor.NewChannelTellOnlyRef[SpendEvent](
		"spend-notify", 1,
	)
	doneNotifier := actor.NewChannelTellOnlyRef[SpendDoneEvent](
	...
	result := spendActor.Receive(t.Context(), &RegisterSpendRequest{
		CallerID:    "test-spend-early-done",
		Outpoint:    &outpoint,
		PkScript:    []byte{0x00, 0x14},
		NotifyActor: fn.Some(spendRef),
		NotifyDone:  fn.Some(doneRef),
	...
	backend.spendDoneChan <- struct{}{}
	_, ok := doneNotifier.AwaitMessage(50 * time.Millisecond)
	require.False(t, ok, "Done overtook the spend observation")

	spendingTx := wire.NewMsgTx(2)
	spendingTx.AddTxIn(&wire.TxIn{Sequence: 1})
	spender := spendingTx.TxHash()
	backend.spendChan <- &SpendDetail{
		SpentOutPoint:  &outpoint,
		SpenderTxHash:  &spender,
		SpendingTx:     spendingTx,
		SpendingHeight: 150,
	}
	spent, ok := spendNotifier.AwaitMessage(awaitTimeout)
	require.True(t, ok, "timeout waiting for SpendEvent")
	require.Equal(t, spender, spent.SpendingTxid)
	done, ok := doneNotifier.AwaitMessage(awaitTimeout)
	require.True(t, ok, "timeout waiting for deferred SpendDoneEvent")
	require.Equal(t, outpoint, done.Outpoint)
}
...
// TestSpendActorRearmsFinalityAfterEpochStreamClose is the spend-side
// equivalent of TestConfActorRearmsFinalityAfterEpochStreamClose.
func TestSpendActorRearmsFinalityAfterEpochStreamClose(t *testing.T) {
	t.Parallel()
	...
	outpoint := wire.OutPoint{Hash: chainhash.Hash{0x12}, Index: 1}
	spendNotifier := actor.NewChannelTellOnlyRef[SpendEvent](
		"spend-rearm-notify", 1,
	)
	reorgNotifier := actor.NewChannelTellOnlyRef[SpendReorgedEvent](
		"spend-rearm-reorged", 1,
	)
	doneNotifier := actor.NewChannelTellOnlyRef[SpendDoneEvent](
		"spend-rearm-done", 1,
	)
	...
	result := spendActor.Receive(t.Context(), &RegisterSpendRequest{
		CallerID: "test-spend-finality-rearm",
		Outpoint: &outpoint,
		PkScript: []byte{0x51},
		NotifyActor: fn.Some[actor.TellOnlyRef[SpendEvent]](
			spendNotifier,
		),
		NotifyReorged: fn.Some[actor.TellOnlyRef[SpendReorgedEvent]](
			reorgNotifier,
		),
		NotifyDone: fn.Some[actor.TellOnlyRef[SpendDoneEvent]](
			doneNotifier,
		),
	})
	require.True(t, result.IsOk())
