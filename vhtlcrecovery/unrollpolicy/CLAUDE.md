...
- `ExitSpendPolicyResolver` — resolves an unroll `(exit_policy_kind,
  exit_policy_ref)` pair into a vHTLC-specific exit policy. Implements
  `unroll.ExitSpendPolicyResolver`. Registered with the unroll registry at
  daemon startup.
...
- `PreimageResolverRegistry` — concurrency-safe late-binding registry for the
  preimage resolver. The unroll subsystem initializes before the swap runtime
  registers its swap store, so the registry holds the slot and the subserver
  installs the concrete resolver later in startup.

## Relationships

- **Depends on**:
  - `vhtlcrecovery` (RecoveryJob row type and constants)
  - `unroll` (ExitSpendPolicy, ExitSpendPolicyResolver, ExitSpendRequest
    interfaces)
  - `lib/arkscript` (VHTLCPolicy script construction and spend path derivation)
  - `lib/tx/arktx` (TxVersion constant for exit transaction construction)
- **Depended on by**: `darepod` (registers `ExitSpendPolicyResolver` with the
  unroll registry and installs the preimage resolver via
  `PreimageResolverRegistry.SetResolver`)
- **Sends**: nothing — policy constructor and spend builder only.
- **Receives**: nothing — called synchronously by the unroll FSM.
