...
- **Depends on**: `btcd/wire` (OutPoint type for VTXO outpoint).
- **Depended on by**: `db` (stores RecoveryJob rows),
  `vhtlcrecovery/coordinator` (runtime service),
  `vhtlcrecovery/unrollpolicy` (adapter to unroll exit policy),
  `darepod` (wires the subsystem, RPC layer).
