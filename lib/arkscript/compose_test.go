...
func TestComposeWithSiblingRootSpendInfo(t *testing.T) {
	...
	// Control block should be longer than the original due to extra sibling.
...
func TestComposeWithSiblingRootDeterministic(t *testing.T) {
	...
	composed1, err := ComposeWithSiblingRoot(policy.CompiledPolicy, externalRoot)
	require.NoError(t, err)

	composed2, err := ComposeWithSiblingRoot(policy.CompiledPolicy, externalRoot)
	require.NoError(t, err)
...
func TestComposedRootOrdering(t *testing.T) {
	...
	composed1, err := ComposeWithSiblingRoot(policy.CompiledPolicy, extRoot1)
	require.NoError(t, err)

	composed2, err := ComposeWithSiblingRoot(policy.CompiledPolicy, extRoot2)
	require.NoError(t, err)
...
func TestComposeWithSiblingRootPreservesWitnessScript(t *testing.T) {
	...
	for i := 0; i < len(policy.Leaves); i++ {
		...
		require.Equal(t, originalInfo.WitnessScript, composedInfo.WitnessScript,
			"leaf %d witness script should be preserved", i)

		// Tx context should also be preserved.
		require.Equal(t,
			originalInfo.RequiredSequence, composedInfo.RequiredSequence,
			"leaf %d RequiredSequence should be preserved", i)
		require.Equal(t,
			originalInfo.RequiredLockTime, composedInfo.RequiredLockTime,
