...
// maxTypesMessageSize bounds the total wire size of an untrusted types TLV
// payload before any individual record length is honored. The largest
// legitimate payload is a join-round auth message: up to
// joinRoundAuthMaxRequestCount (1024) entries per list at
// joinRoundAuthMaxBlobEntrySize (64 KiB) each, across four lists, plus the
// TxProof codec which embeds a full transaction and merkle proof. 16 MiB gives
// generous headroom for the largest legitimate body while still rejecting the
// unbounded-allocation DoS where a tiny payload declares a multi-gigabyte (or
// near-2^64) record length.
//
// These payloads cross the client/server trust boundary (join-auth is the
...
// safeTypesTLVBytes validates an in-memory TLV blob and returns a reader
...
// the input on the non-P2P Decode / DecodeWithParsedTypes path. A
// producer-declared length near 2^64 panics the decoder with "makeslice: cap
// out of range", and a multi-gigabyte length drives an OOM. By rejecting any
// record whose declared length exceeds the remaining buffer (a record can
// never legitimately carry more bytes than the message contains), we cap every
// allocation at the message size, which is itself capped at
// maxTypesMessageSize.
func safeTypesTLVBytes(raw []byte) (*bytes.Reader, error) {
	if len(raw) > maxTypesMessageSize {
		return nil, fmt.Errorf("%w: message %d bytes exceeds max %d",
			ErrInvalidTLV, len(raw), maxTypesMessageSize)
	}
