...
func buildTaprootWitness(in psbt.PInput,
	...
	if len(in.TaprootScriptSpendSig) > 0 {
		...
		case errors.Is(err, arkscript.ErrConditionWitnessNotFound):
			// Plain collaborative leaves do not carry extra condition
			// witness material, so missing Ark condition metadata is fine.
...
func orderTaprootScriptSpendSignatures(
	...
	if len(keys) == 0 {
		...
		for i := range sigs {
			...
			ordered = append(
				ordered,
				appendTaprootSigHash(sig.Signature, sig.SigHash),
			)
		}
	...
	if len(keys) != len(sigs) {
		return nil, fmt.Errorf(
			"taproot signature count %d does not match leaf key count %d",
			len(sigs), len(keys),
		)
	}
	...
	for i := len(keys) - 1; i >= 0; i-- {
		...
		if !ok {
			return nil, fmt.Errorf(
				"missing taproot signature for witness key",
			)
		}
