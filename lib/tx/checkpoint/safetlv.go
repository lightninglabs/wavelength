...
// huge length panics (makeslice) or OOMs. We pre-validate framing to fail
// closed instead.
...
// bytes for the real decode.
//
// The check is intentionally cheap: every record costs at least its declared
// length in payload bytes, so a record claiming more bytes than remain in the
// buffer is definitionally malformed. Rejecting it up front means the
...
func validateTLVFraming(raw []byte) error {
	...
	for reader.Len() > 0 {
		// The type varint is part of framing, not payload; an error
		// here is a normal short read that the real decoder will also
		// surface, so we stop validating and let it produce the
		// canonical error.
