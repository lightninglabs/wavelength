...
type tlvNode struct {
	// Amount preserves the cached subtree BTC amount in new checkpoints.
	Amount tlv.RecordT[tlvNodeAmount, uint64]
...
func newTlvNode() *tlvNode {
	return &tlvNode{
		Amount: tlv.NewPrimitiveRecord[tlvNodeAmount, uint64](0),
...
func (n *tlvNode) EncodeRecords() []tlv.Record {
	...
	if n.Amount.Val != 0 {
		records = append(records, n.Amount.Record())
	}
...
func SerializeTree(t *tree.Tree) ([]byte, error) {
	return serializeTree(t, false)
}

// SerializeSnapshot additionally preserves cached subtree amounts. These
// fields stay out of SerializeTree because its bytes identify ancestry rows.
func SerializeSnapshot(t *tree.Tree) ([]byte, error) {
	return serializeTree(t, true)
}

// serializeTree shares the node representation while keeping legacy fragment
// identities independent of cached subtree amounts.
func serializeTree(t *tree.Tree, snapshot bool) ([]byte, error) {
	if t == nil {
	...
	if t.Root != nil {
		rootData, err := serializeNode(t.Root, t.AssetContext, snapshot)
		if err != nil {
...
func serializeNode(n *tree.Node, assetCtx *tree.AssetTreeContext,
	...
	tlvN := newTlvNode()
	if snapshot {
		tlvN.Amount.Val = uint64(n.Amount)
	}
	...
	childrenData, err := serializeChildren(n.Children, assetCtx, snapshot)
	if err != nil {
...
func deserializeNode(data []byte, depth int,
	...
	n := &tree.Node{
		Amount:    btcutil.Amount(int64(tlvN.Amount.Val)),
...
func serializeChildren(children map[uint32]*tree.Node,
	assetCtx *tree.AssetTreeContext, snapshot bool) ([]byte, error) {
	...
	for _, idx := range indices {
		...
		childData, err := serializeNode(child, assetCtx, snapshot)
		if err != nil {
