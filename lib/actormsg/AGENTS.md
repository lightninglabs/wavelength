...
- `ReleaseSpendRequest` / `CompleteSpendRequest` — Spend lifecycle completion messages. `ReleaseSpendRequest.ReserveEpochs` optionally names, per outpoint, the reservation epoch the releasing owner held, so a superseded release is refused instead of freeing a coin a newer session already owns (see Invariants).
...
- `SelectedVTXO` — Describes a VTXO selected for spend (outpoint, amount, pkscript, `ReserveEpoch`). The epoch is the manager's monotonic reservation counter for that VTXO, echoed back so the spend that carries the coin into its durable state can present it on release. It is zero for reservations that are not spend reservations (e.g. forfeit inputs).
...
- **A reservation epoch makes release idempotent under redelivery, not just
  ordered.** `SelectedVTXO.ReserveEpoch` flows out of the manager on
  reservation and back in via `ReleaseSpendRequest.ReserveEpochs`;
  `vtxo.Manager.handleReleaseSpend` releases only when the VTXO's current
  epoch still matches. Without it, a rolled-back failure that redelivers a
  release can free a coin that was already released and re-reserved by a
  newer session, returning a coin that session is actively spending to the
  live set.
- **An absent epoch releases unconditionally.** A nil `ReserveEpochs` map, or
  a map with no entry for an outpoint, preserves the original behaviour for
  callers that legitimately hold no epoch (e.g. a manual operator unlock). Do
  not make the map required — that would break those paths — and do not treat
  a missing entry as epoch zero.
