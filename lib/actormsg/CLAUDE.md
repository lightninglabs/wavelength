...
- `ReleaseSpendRequest` / `CompleteSpendRequest` — Spend lifecycle completion messages. `ReleaseSpendRequest.ReserveEpochs` optionally names, per outpoint, the reservation epoch the releasing owner held (see the reservation-epoch invariant below).
...
- **The reservation epoch is an ABA guard, and zero means "unknown".**
  `SelectAndReserveSpendResponse` stamps `SelectedVTXO.ReserveEpoch` with the
  VTXO manager's monotonic epoch for that reservation. The spend carries it
  into its durable state (`wallet.SelectedVTXO` →
  `oor.TransferInput.ReserveEpoch` → the persisted snapshot) and presents it
  back in `ReleaseSpendRequest.ReserveEpochs`, so the manager can refuse a
  stale release whose reservation was already superseded by a newer session
  on the same outpoint. A zero epoch or an absent map entry releases
  unconditionally, preserving the pre-epoch behaviour for callers that hold
  no epoch (a manual unlock, a pre-upgrade snapshot). Forfeit reservations
  do not use it and leave it zero. Any new field carrying a reservation
  across a durable boundary must carry the epoch with it, or the guard has a
  hole.
