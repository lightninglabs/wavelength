...
- `ReleaseSpendRequest` / `CompleteSpendRequest` — Spend lifecycle completion messages. `ReleaseSpendRequest.ReserveEpochs` optionally names, per outpoint, the reservation epoch the releasing owner held, so a stale release is refused rather than honoured (see the epoch invariant below).
...
- `SelectedVTXO` — Describes a VTXO selected for spend (outpoint, amount, pkscript). Also carries `ReserveEpoch`, the manager's monotonic reservation epoch for this VTXO, echoed back so the spend that carries it into its durable state can name the reservation on release. Zero for reservations that are not spend reservations (e.g. forfeit inputs).
...
- **Spend releases are epoch-checked, not unconditional.** A spend reservation
  hands the owner a monotonic `ReserveEpoch`; the owner threads it through its
  durable state and presents it back in `ReleaseSpendRequest.ReserveEpochs`.
  The manager releases the VTXO only if the coin's current epoch still matches.
  Without this, a release redelivered after a rolled-back failure — from a
  session whose reservation was already superseded by a newer one — would
  return a coin the newer session is actively spending back to the live set.
  An absent entry (or a nil map) releases unconditionally, preserving the
  behaviour for callers that hold no epoch, e.g. a manual unlock.
