...
- `Proof.RootExternalInputs` — The outpoints root transactions consume that no
  node in the proof produces: the external funding the whole graph hangs off
  of. For a round-direct VTXO that is the batch/commitment output the tree root
  spends; for an OOR-chained or multi-input fan-in VTXO it is every distinct
  commitment output rooting a local lineage fragment.
...
- `RootExternalInputs` names exactly the outpoints a competing party (an
  ...
  the exit. A confirmed foreign spend of any one of them makes every root that
  depends on it — and therefore the whole proof — permanently unbroadcastable,
  so callers arm spend watches on them and fail the exit terminally instead of
  spinning forever on a tree that can never confirm. The result is deduplicated
  and sorted by hash then index, so two proofs built from the same node set
  yield identical output regardless of map iteration order.
