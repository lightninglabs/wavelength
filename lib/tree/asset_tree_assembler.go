...
// can spend it).
//
// Returns the assembled tree and an AssetContext containing asset-specific
// state (taproot tweaks, proofs) for each node. The context should be used
// with TweakLookupFromAssetContext during signing to provide node-specific
// tweaks.
func (a *AssetTreeAssembler) BuildTree(ctx context.Context,
	...
	*AssetContext, error) {
	...
	if len(leaves) == 0 {
		return nil, nil, fmt.Errorf("no leaves supplied")
	}
	if rootOutput == nil {
		return nil, nil, fmt.Errorf("root output cannot be nil")
	}
	...
	if err != nil {
		return nil, nil, fmt.Errorf("build tree structure: %w", err)
	}
	root := result.Root
	...
	// Pass the asset context from structure building to the materializer.
	mat := NewAssetMaterializer(a.cfg, a.wallet, result.AssetContext)

	matParams := MaterializeParams{
		Input:         rootInput,
		ParentProof:   rootProof,
		ParentPlan:    rootPlan,
		InputBtcValue: int64(root.Amount),
	}

	if err := Materialize(ctx, root, matParams, mat); err != nil {
		return nil, nil, fmt.Errorf("materialize tree: %w", err)
	}
...
// proof (the input being spent) and stores it in the asset context. This
// ensures tree signing uses the same tweak that tapd applied when creating the
// parent's output (which is now this node's input). It also sets node.FinalKey
// to the actual output key from the proof. When the proof omits the tapscript
// sibling (common for script-controlled VTXOs), the sibling is reconstructed
// from the anchor plan's script plans.
//
// The tweak is stored in the asset context in two ways:
// 1. By node pointer (via AssetNodeState.TaprootTweak) for direct lookup
// 2. By node input outpoint (via SetTweakByOutpoint) for lookup during signing
//
// The outpoint-keyed lookup is necessary because ExtractPathForCoSigner creates
// new node objects with different pointer identity but the same input outpoint.
// TweakLookupFromAssetContext uses the outpoint-keyed map to retrieve tweaks
// during signing.
func attachTaprootTweakFromParent(node *Node, parentPlan *assets.AnchorPlan,
	parentProof []byte, assetCtx *AssetContext) error {
	...
	commitProof := lastProof.InclusionProof.CommitmentProof
	if commitProof != nil && commitProof.TapSiblingPreimage != nil {
	...
	// and the tweak from asset context, and it should produce the same key.
	...
	// Store tweak in the asset context. Asset trees use
	// TweakLookupFromAssetContext to retrieve tweaks during signing. The
	// asset context is keyed by node pointer, so we update the existing
	// entry if present.
	if assetCtx != nil {
		state := assetCtx.Get(node)
		if state == nil {
			state = &AssetNodeState{}
		}

		state.TaprootTweak = taprootRoot
		assetCtx.Set(node, state)

		// Also store the tweak by outpoint for lookup during signing.
		// This is needed because ExtractPathForCoSigner creates new
		// node objects with different pointer identity but the same
		// input outpoint.
		assetCtx.SetTweakByOutpoint(node, taprootRoot)
	}
...
func taprootKeyFromScript(pkScript []byte) (*btcec.PublicKey, error) {
	if len(pkScript) != 34 {
		return nil, fmt.Errorf(
			"unexpected pkscript len %d", len(pkScript),
		)
	}
