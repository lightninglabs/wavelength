...
// TestSignerSessionTweakLookup ensures an asset signing lookup is keyed by
// input outpoint and fails closed if any node lacks its required tweak.
func TestSignerSessionTweakLookup(t *testing.T) {
	...
	t.Run("multiple lookups rejected", func(t *testing.T) {
		t.Parallel()

		root, _, _ := newTweakTestPath(t, pubKey)
		signer := &recordingMuSig2Signer{
			mockMuSig2Signer: newMockMuSig2Signer(privKey),
		}
		fetcher, err := root.PrevOutputFetcher(
		...
		lookup := func(*Node) []byte { return nil }
		_, err = NewSignerSession(
			signer, &keychain.KeyDescriptor{
				PubKey: pubKey,
			},
			sweepRoot,
			fetcher,
