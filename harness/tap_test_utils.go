...
func (tc *TapClientHarness) SyncUniverse() {
	...
	// Now manually trigger a full sync. Retry if sync is already in
	// progress (can happen when multiple clients sync concurrently).
	var syncErr error
	for i := 0; i < 10; i++ {
		_, syncErr = tc.TapdClient.SyncUniverse(
			ctx, &universerpc.SyncRequest{
				UniverseHost: universeHost,
				SyncMode:     universerpc.UniverseSyncMode_SYNC_FULL, //nolint:ll
			},
		)
		if syncErr == nil {
			break
		...
		if strings.Contains(syncErr.Error(), syncInProgress) {
...
// SyncUniverseWith syncs this tapd instance with a specific universe host.
func (tc *TapClientHarness) SyncUniverseWith(universeHost string) {
	tc.t.Helper()
	...
	// Ignore duplicate/unusable errors.
	if err != nil {
		msg := err.Error()
		switch {
		case strings.Contains(
	...
	for i := 0; i < 10; i++ {
		_, syncErr = tc.TapdClient.SyncUniverse(
			ctx, &universerpc.SyncRequest{
				UniverseHost: universeHost,
				SyncMode: universerpc.
					UniverseSyncMode_SYNC_FULL,
			},
		)
		if syncErr == nil {
			break
