...
// Logf prints the formatted test log message.
...
func (h *Harness) removeContainerByName(name string) {
	...
	for _, container := range containers {
		...
		_ = h.pool.Client.KillContainer(docker.KillContainerOptions{
			ID: container.ID,
		})

		// Remove the container.
		_ = h.pool.Client.RemoveContainer(docker.RemoveContainerOptions{
			ID:    container.ID,
			Force: true,
		})
	}
...
// Generate mines n blocks to a fresh regtest address.
...
func (h *Harness) GenerateAndWait(numBlocks int) []Block {
	...
	if wait := time.Until(blockTime); wait > 0 {
		...
		// Add 1 second cushion beyond block time to ensure any
		// time-dependent operations have margin for clock skew and
		// processing delays.
		const cushion = 1000 * time.Millisecond
		time.Sleep(wait + cushion)
	}

	return blocks
}

// BlockCount returns the current regtest height according to bitcoind.
...
// Faucet sends amount BTC to address using bitcoind's wallet.
...
// MempoolTxIDs returns the txids of all transactions currently in the mempool.
