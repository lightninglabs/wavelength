...
// receiveInvoiceKeyFileName is the temporary daemon-local receive invoice key
// file name. TODO(#337): replace this first-slice local key persistence with a
// wallet-derived key once receive-swap mailbox auth is wired into WalletDK.
const receiveInvoiceKeyFileName = "receive_invoice_key"
...
// in-process Ark SDK facade over darepod's existing DaemonService, configures
// the first-slice receive invoice key, and wires the sdk/swaps client. It also
// returns a cleanup function that must be called during daemon shutdown so the
// root worker context is canceled before the Ark, swapdk-server, and store
...
func newSwapClientService(ctx context.Context, rpcServer *darepod.RPCServer,
	...
	*swapClientService, func(), error) {
	...
	invoiceKey, err := loadOrCreateReceiveInvoiceKey(
		receiveInvoiceKeyPath(dbPath),
	)
	if err != nil {
		_ = arkClient.Close()
		_ = serverConn.Close()
	...
	swapClient := swaps.NewSwapClientWithStore(
		serverConn, arkClient, log,
		daemonInvoiceGenerator(invoiceKey, arkClient, chainParams),
		store,
	)
...
// daemonInvoiceGenerator constructs the invoice creator used by receive swaps.
// It uses a daemon-scoped signing key for this first slice and asks the local
// daemon RPC surface for the current block height whenever sdk/swaps needs to
// encode invoice expiry data.
func daemonInvoiceGenerator(invoiceKey *btcec.PrivateKey,
	arkClient *sdkark.Client,
...
// receiveInvoiceKeyPath returns the daemon-local path that keeps receive swap
// invoice authentication stable across daemon restarts.
func receiveInvoiceKeyPath(dbPath string) string {
	return filepath.Join(filepath.Dir(dbPath), receiveInvoiceKeyFileName)
}

// loadOrCreateReceiveInvoiceKey returns the persisted daemon receive invoice
// signing key, creating it on first use. The key is intentionally process-local
// daemon state for this first slice so receive swaps created before a restart
// can still be authenticated after restart.
func loadOrCreateReceiveInvoiceKey(
	path string) (*btcec.PrivateKey, error) {

	raw, err := os.ReadFile(path)
	if err == nil {
		if len(raw) != btcec.PrivKeyBytesLen {
			return nil, fmt.Errorf(
				"receive invoice key has %d bytes, want %d",
				len(raw), btcec.PrivKeyBytesLen,
			)
		}
