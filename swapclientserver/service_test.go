...
func TestLoadOrCreateReceiveInvoiceKeyPersistsKey(t *testing.T) {
	t.Parallel()

	keyPath := filepath.Join(t.TempDir(), receiveInvoiceKeyFileName)

	key, err := loadOrCreateReceiveInvoiceKey(keyPath)
	require.NoError(t, err)

	reloaded, err := loadOrCreateReceiveInvoiceKey(keyPath)
	require.NoError(t, err)
	require.Equal(t, key.Serialize(), reloaded.Serialize())
}
