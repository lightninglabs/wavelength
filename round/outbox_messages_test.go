...
func TestOutboxMessagesToProto(t *testing.T) {
	...
	t.Run("SubmitNoncesRequest_ToProto", func(t *testing.T) {
		...
		msg := &SubmitNoncesRequest{
			...
			Nonces:         [][]byte{{0x01, 0x02}, {0x03, 0x04}},
		}
...
// TestOutboxMessagesClientEventSealed ensures that all outbox message types
// implement the ClientEvent sealed interface. The clientEventSealed() method
// acts as a compile-time marker preventing external types from implementing
// ClientEvent, so this test verifies the marker exists on all expected types.
func TestOutboxMessagesClientEventSealed(t *testing.T) {
	t.Parallel()
	...
	t.Run("AddressCreated", func(t *testing.T) {
		t.Parallel()
		msg := &AddressCreated{
			Address:     addr,
			ClientKey:   pubKey,
