...
- `RefreshVTXORequest` — Per-VTXO refresh registration carrying `Amount`, `VTXOOutpoint`, `SigningKey`, `OwnerKey`, `PolicyTemplate`, and `OperatorFee int64`. Under the #270 seal-time fee handshake, `OperatorFee` is an **advisory-only** hint filled by the VTXO actor's `RefreshFeeQuoter` for observability (logs, metrics); `buildVTXORequestFromRefresh` does NOT subtract it from `Amount`. Instead, the new VTXO request is emitted with `IsChange=true` so the server stamps the residual at seal time. Zero is valid (quota unavailable or zero-fee schedule).
