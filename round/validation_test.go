...
func TestValidateVTXTPathNilTree(t *testing.T) {
	t.Parallel()

	h := newTestHarness(t)
	vtxoRequests := []types.VTXORequest{{Amount: 50000}}

	_, err := ValidateVTXTPath(
		nil, h.clientPubKey, vtxoRequests, h.operatorPubKey,
	)
	require.Error(t, err)
	require.Contains(t, err.Error(), "tree is nil")
}

func TestValidateVTXTPathNilSigningKey(t *testing.T) {
	t.Parallel()
	...
	_, err := ValidateVTXTPath(
		mockTree, nil, vtxoRequests, h.operatorPubKey,
	)
	require.Error(t, err)
	require.Contains(t, err.Error(), "signing key is nil")
}

func TestValidateVTXTSignaturesNilTree(t *testing.T) {
	t.Parallel()

	signatures := [][]byte{make([]byte, 64)}

	err := ValidateVTXTSignatures(nil, signatures)
	require.Error(t, err)
	require.Contains(t, err.Error(), "tree is nil")
}

func TestValidateVTXTSignaturesEmptySignatures(t *testing.T) {
	t.Parallel()

	mockTree := &tree.Tree{}

	err := ValidateVTXTSignatures(mockTree, [][]byte{})
	require.Error(t, err)
	require.Contains(t, err.Error(), "no signatures provided")
}

func TestValidateVTXTSignaturesNilSignatures(t *testing.T) {
	t.Parallel()

	mockTree := &tree.Tree{}

	err := ValidateVTXTSignatures(mockTree, nil)
	require.Error(t, err)
	require.Contains(t, err.Error(), "no signatures provided")
}

func TestValidateAnchorOutputsNilTree(t *testing.T) {
	t.Parallel()

	err := ValidateAnchorOutputs(nil)
	require.Error(t, err)
	require.Contains(t, err.Error(), "tree is nil")
}
...
// createTestVTXTTree would create a minimal VTXT tree for testing, but actual
// tree construction requires full lib/tree support which is not yet available.
//
//nolint:unused
func createTestVTXTTree(
	t *testing.T, clientKey, operatorKey *btcec.PublicKey,
) *tree.Tree {
