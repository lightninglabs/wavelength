...
// MockChainBackend implements ChainBackend using mock.Mock for testing.
type MockChainBackend struct {
	mock.Mock
}

func (m *MockChainBackend) BroadcastTx(tx *wire.MsgTx) error {
	args := m.Called(tx)
	return args.Error(0)
}

//nolint:forcetypeassert
func (m *MockChainBackend) GetBestBlock() (*chainhash.Hash, int32, error) {
	args := m.Called()

	var hash *chainhash.Hash
	if args.Get(0) != nil {
		hash = args.Get(0).(*chainhash.Hash)
	}

	return hash, args.Get(1).(int32), args.Error(2)
}

// Compile-time check that MockChainBackend implements ChainBackend.
var _ ChainBackend = (*MockChainBackend)(nil)
...
func newTestHarness(t *testing.T) *boardingTestHarness {
	...
	h := &boardingTestHarness{
		...
		currentState:    &Idle{},
		outboxMessages:  make([]ClientEvent, 0),
	}
...
func (h *boardingTestHarness) newNoncesAggregatedEvent(
	...
	aggNonces := make([][]byte, txCount)
	for i := range aggNonces {
		aggNonces[i] = make([]byte, musig2.PubNonceSize)
		_, err := rand.Read(aggNonces[i])
		require.NoError(h.t, err)
	}
...
// setupMockWalletForMuSig2Session configures the wallet mock to return a valid
// MuSig2 session for testing nonce generation without real cryptographic setup.
func (h *boardingTestHarness) setupMockWalletForMuSig2Session() {
	h.t.Helper()
	...
	mockCall := h.wallet.On(
		"MuSig2CreateSession", mock.Anything, mock.Anything,
		mock.Anything, mock.Anything, mock.Anything, mock.Anything,
	)
	mockCall.Return(sessionInfo, nil)
}

func (h *boardingTestHarness) setupMockWalletForNonceRegistration() {
	h.t.Helper()
...
// setupMockWalletForSigning configures the wallet mock to return mock partial
// signatures during the MuSig2 signing phase.
func (h *boardingTestHarness) setupMockWalletForSigning() {
	h.t.Helper()
	...
	mockCall.Return(partialSig, nil)
}
...
//nolint:unused
func (h *boardingTestHarness) setupMockWalletForFullSigningFlow() {
	h.t.Helper()

	h.setupMockWalletForMuSig2Session()
	h.setupMockWalletForNonceRegistration()
...
func (h *boardingTestHarness) newInputSigSentState(
	...
	for _, intent := range intents {
		for _, vtxoReq := range intent.VtxoTemplate {
			sigKey := vtxoReq.SigningKey.PubKey
			clientID := signerKeyIDFromPubKey(sigKey)
			clientTrees[clientID] = vtxtTree
		}
	}
...
func signerKeyIDFromPubKey(pk *btcec.PublicKey) string {
	if pk == nil {
		return ""
	}

	return fmt.Sprintf("%x", pk.SerializeCompressed())
}
