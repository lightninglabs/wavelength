...
//
// It is deliberately unconditional across failure paths. A round that never
// reached the checkpoint has no row to retire and no adopted intents, so both
// halves are no-op UPDATEs; gating on "was this round checkpointed" would
// duplicate that knowledge in the actor for no gain. The intent revert is
// itself guarded on 'adopted' in SQL, so a sweep that already moved a deposit
// on is never clobbered.
//
