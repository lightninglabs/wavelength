...
// type used throughout the client boarding FSM. The baselib protofsm uses 3
// type parameters: InternalEvent, OutboxEvent, and Env. In our case:
//   - InternalEvent = ClientEvent (events that drive the FSM).
//   - OutboxEvent = ClientEvent (outbox messages emitted by transitions).
//   - Env = *ClientEnvironment.
//
//nolint:ll
type ClientStateTransition = protofsm.StateTransition[
	ClientEvent, ClientEvent, *ClientEnvironment,
]
...
type ClientEmittedEvent = protofsm.EmittedEvent[ClientEvent, ClientEvent]

// ClientStateMachine is a type alias for the client boarding FSM.
...
// ChainBackend abstracts blockchain operations like transaction broadcasting.
// This is separate from chain monitoring to distinguish between active
// operations (broadcasting) and passive monitoring (confirmations, spends).
//
// Used by both client and server FSMs. The actor layer implements this using
// the chainsource actor's BroadcastTx message.
type ChainBackend interface {
	// BroadcastTx broadcasts a transaction to the Bitcoin network. Returns
	// an error if the transaction is rejected by the network (e.g., double
	// spend, invalid signature, insufficient fee).
	BroadcastTx(tx *wire.MsgTx) error

	// GetBestBlock returns the current best block hash and height. This is
	// used for fee estimation and determining height hints for monitoring.
...
type Round struct {
	...
	// CreationHeight is the block height when this round was created.
	...
	// VTXTTree is the complete Virtual Transaction Tree. This is ephemeral
	// and only kept in memory during signing. After confirmation, only
	// extracted paths are persisted.
